...
/**
 * Fragment that is the push notification manage screen. Uses the push save
 * header, push toggle item and push save item. It will compile them into the
 * list.
 * 
 * @author jthornton
 * 
...
public class PushManageFragment extends BaseFragment {
    ...
    /**
     * Linear layout holding a list of toggle items for the monitor your
     * spending
     */
    ...
    public View onCreateView(final LayoutInflater inflater,
        ...
        final View mainView = inflater.inflate(
                R.layout.push_manage_main_layout, null);
     ...
     * 
     * @param mainView
     *            - view holding the items that need the click handlers
     */
    private void setClickHandlers(final View mainView) {
        final TextView termsLaunch = (TextView) mainView
                .findViewById(R.id.clickable_view);
        final Button save = (Button) mainView
                .findViewById(R.id.notification_save_button);
     ...
     * 
     * @param mainView
     *            - view holding the class layouts
     */
    private void extractClassLayouts(final View mainView) {
        manageHeader = (PushManageHeaderItem) mainView
                .findViewById(R.id.manage_you_account_header);
        manageList = (LinearLayout) mainView
                .findViewById(R.id.manage_account_list_preferences);
        monitorHeader = (PushManageHeaderItem) mainView
                .findViewById(R.id.monitor_spending_list_header);
        monitorList = (LinearLayout) mainView
                .findViewById(R.id.monitor_spending_list_preferences);
        maximizeHeader = (PushManageHeaderItem) mainView
                .findViewById(R.id.maximize_rewards_list_header);
        maximizeList = (LinearLayout) mainView
                .findViewById(R.id.maximize_rewards_list_preferences);
        phoneNumber = (TextView) mainView.findViewById(R.id.phone_number);
        enableText = (ImageView) mainView
                .findViewById(R.id.toggle_enable_texts);
     ...
     * 
     * @param image
     *            - imageview of the state to switch
     * @param isEnabled
     *            - if the image view is currently enabled
     */
    ...
    /**
     * On resume of the page make sure the fragment gets an updated version of
     * the prefs
     */
    @Override
    public void onResume() {
        ...
        final AsyncCallback<PushNotificationPrefsDetail> callback = GenericAsyncCallback
                .<PushNotificationPrefsDetail> builder(this.getActivity())
                .showProgressDialog(
                ...
                .withErrorResponseHandler(
                        new GetPushPrefsErrorResponseHandler())
     ...
     * @param categories
     *            - list of categories to associate with the items
     * @param headers
     *            - list of headers to display in the items
     * @param texts
     *            - list of text values to display in the items
     */
    private void createList(final LinearLayout list, final String[] categories,
            final String[] headers, final String[] texts) {

        final int lengthOfHeaders = headers.length;

        for (int i = 0; i < lengthOfHeaders; i++) {
            final PushManageCategoryItem item = factory.createItem(
                    categories[i], headers[i], texts[i]);
    ...
    private void createMonitorList() {
        ...
        final int tamtDefined = getAmountFromPrefsToPlaceInEditText(
        ...
        final PushManageCategoryItem purchase = factory.createItem(
                res.getString(R.string.purchase_amount_category),
                res.getString(R.string.purchase_amount_header), tamtDefined,
                prefs.remindersEnrollResults.tamtMinAmt,
                prefs.remindersEnrollResults.tamtMaxAmt, this);

        final int balanceDefined = getAmountFromPrefsToPlaceInEditText(
        ...
        final PushManageCategoryItem balance = factory.createItem(
                res.getString(R.string.balance_amount_category),
                res.getString(R.string.balance_amount_header), balanceDefined,
                prefs.remindersEnrollResults.balanaceMinAmt,
                prefs.remindersEnrollResults.balanaceMaxAmt, this);

        final int creditLineAmount = getAmountFromPrefsToPlaceInEditText(
                res.getString(R.string.credit_line_ammount_category),
                prefs.remindersEnrollResults.crltDefAmt);

        final int creditLineIndex = getDropdownIndexFromAmount(
                creditLineAmount, prefs.remindersEnrollResults.crltAmtOptions);
        ...
        final PushManageCategoryItem creditLine = factory.createItem(
                ...
                prefs.remindersEnrollResults.crltAmtOptions, creditLineIndex,
                this);
    ...
    /**
     * Get the index of the amount from the list so that it can be displayed in
     * a dropdown correctly
     * 
     * @param amount
     *            - ammount to get from list
     * @param options
     *            - list of options
     * @return the index of the amount in the list
     */
    private int getDropdownIndexFromAmount(final int amount,
    ...
    private void createRewardsList() {
        ...
        final int mrrwDefAmount = getAmountFromPrefsToPlaceInEditText(
        ...
        final PushManageCategoryItem cashBackBonus = factory.createItem(
                res.getString(R.string.cashback_bonus_category),
                res.getString(R.string.cashback_bonus_header), mrrwDefAmount,
                prefs.remindersEnrollResults.mrrwMinAmt,
                prefs.remindersEnrollResults.mrrwMaxAmt, this);
    ...
    private void createHeaders() {
        manageHeader.setHeader(res
                .getString(R.string.manage_your_account_title));
        monitorHeader.setHeader(res
                .getString(R.string.monitor_your_spending_title));
        maximizeHeader.setHeader(res
                .getString(R.string.maximize_your_rewards_title));
    }
    ...
    public int getActionBarTitle() {
        return R.string.sub_section_title_manage_alerts;
    }
    ...
    public void displayPrefs() {
        ...
        phoneNumber.setText(CommonUtils
                .toPhoneNumber(prefs.remindersEnrollResults.phoneNumber));
    ...
    public void savePreferences() {
        final AsyncCallback<PostPreferencesDetail> callback = GenericAsyncCallback
                .<PostPreferencesDetail> builder(this.getActivity())
                .showProgressDialog(
                        getResources().getString(
                                R.string.push_progress_get_title),
                        getResources().getString(
