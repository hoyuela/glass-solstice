 ...
 * 
 * Extending is not required; if a class needs no custom JSON response code
 * handling then this class would be adequate
 * 
 * @author ekaram
 * 
 */
public class CardBaseErrorResponseHandler implements ErrorResponseHandler {
    ...
    public CardBaseErrorResponseHandler(final ErrorHandlerUi errorHandlerUi) {
        ...
        mErrorHandlerFactory = errorHandlerUi.getErrorHandler();
    }
     ...
     */
    @Override
    public final boolean handleFailure(final NetworkServiceCall<?> sender,
        ...
        if (errorResponse instanceof JsonMessageErrorResponse) {
            ...
            case STRONG_AUTH_NOT_ENROLLED:
                mErrorHandlerFactory.handleLockedOut(errorHandlerUi, resources
                        .getString(R.string.account_security_not_enrolled));
                return true;
            ...
            case ACCOUNT_NUMBER_REREGISTERED:
                mErrorHandlerFactory.handleLockedOut(errorHandlerUi, resources
                        .getString(R.string.account_number_reregistered));
                return true;
            ...
            errorHandlingFailureMessage = "ERROR - UNHANDLED JSON ERROR CODE: "
                    + messageErrorResponse.getMessageStatusCode() + " : "
            ...
            errorHandlingFailureMessage = "ERROR - UNHANDLED HTTP ERROR CODE: "
                    + errorResponse.getHttpStatusCode();
        }
        // TODO: This is for development use only; don't want to push this line
        // into production
        showDynamicModalErrorDialog(R.string.generic_error_title,
                errorHandlingFailureMessage, R.string.ok);
        return false;
    }
     ...
     * 
     * @param errorCode
     *            Status Code from the response received
     */
     ...
     * @return
     */
    protected boolean handleJsonErrorCode(
    ...
    protected boolean handleHTTPErrorCode(final int httpErrorCode) {
        ...
        case HttpURLConnection.HTTP_UNAVAILABLE:
            sendToErrorPage(httpErrorCode, R.string.error_503_title,
                    R.string.internal_server_error_503);
            return true;
     ...
     */
    protected void showModalErrorDialog(final int title, final int content,
     ...
     * @param stringResource
     */
    protected void showDynamicModalErrorDialog(final int title,
    ...
    protected void setErrorText(final int errorText) {
        final ErrorHandlerUi errorHandlerUi = getErrorFieldUi();
        if (errorHandlerUi != null) {
            errorHandlerUi.getErrorLabel().setText(
                    ((Context) errorHandlerUi).getResources().getString(
                            errorText));
    ...
    /**
     * Show the default one-button alert with a custom title, content an button
     * text
     * 
     * Uses the orange button
     * 
     * @param title
     *            - the resource id for title for the alert
     * @param content
     *            - the resource id for content to display on the box
     * @param buttonText
     *            - the resource id for button text to display on the button
     */
    public void showOneButtonAlert(final int title, final int content,
            final int buttonText) {
        showCustomAlert(new ModalAlertWithOneButton(getErrorFieldUi()
                .getContext(), title, content, buttonText));
    }
    ...
    protected void sendToErrorPage(final int titleText, final int errorText) {
        ...
        final ErrorHandler handler = getErrorFieldUi().getErrorHandler();
        final AlertDialog dialog = handler.createErrorModal(
                context.getString(titleText), context.getString(errorText));
     ...
     * @param errorText
     */
    protected void sendToErrorPage(final int errorCode, final int titleText,
            final int errorText) {
        final ErrorHandler handler = getErrorFieldUi().getErrorHandler();
        final AlertDialog dialog = handler.createErrorModal(errorCode,
                titleText, errorText);
        handler.showCustomAlert(dialog);
    }
    ...
    protected void sendToErrorPage(final int errorText) {
        ...
        final ErrorHandler handler = getErrorFieldUi().getErrorHandler();
        final AlertDialog dialog = handler.createErrorModal(
                context.getString(R.string.default_outage_header),
