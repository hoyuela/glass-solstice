...
/**
 * StrongAuthDefaultResponseHandler Shows Modal Dialogs on various
 * {@link StrongAuthListener} callback methods
 * 
 * @author CTS
 * 
 * @version 1.0
...
public abstract class StrongAuthDefaultResponseHandler implements
    ...
    public void onStrongAuthSkipped(final Object data) {
        ...
        final EnhancedContentModal modal = new EnhancedContentModal(context,
                R.string.E_SA_SKIPPED_TITLE, R.string.E_SA_SKIPPED_CONTENT,
                R.string.close_text, new Runnable() {
                    @Override
                    public void run() {
    ...
    public void onStrongAuthCardLock(final Object data) {
        final Context context = DiscoverActivityManager.getActiveActivity();
        final EnhancedContentModal modal = new EnhancedContentModal(context,
                R.string.E_T_4031401_LOCKOUT, R.string.E_1402_LOCKOUT,
                R.string.close_text, new Runnable() {
                    @Override
                    public void run() {
                        Utils.logoutUser(
