...
public class ChoosePeriodItem extends RelativeLayout {
     ...
     * @param period
     *            - time period to associate with this item
     */
    public ChoosePeriodItem(final Context context, final AttributeSet attrs,
            final RecentActivityPeriodDetail period) {
        super(context, attrs);

        final RelativeLayout mainView = (RelativeLayout) LayoutInflater.from(
                context).inflate(R.layout.choose_period_item, null);

        /** Label associated with the item */
        final TextView label = (TextView) mainView
                .findViewById(R.id.date_string);
