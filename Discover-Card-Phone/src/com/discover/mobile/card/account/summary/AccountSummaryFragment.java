...
/**
 * Fragment holding the account summary information for both miles and cashback
 * users
 * 
 * @author jthornton
 * 
...
public class AccountSummaryFragment extends BaseFragment {
    ...
    /**
     * Dialog used to show progress, note this has to be done here because their
     * are two calls made back to back. So this will need to keep track of when
     * to show the dialog.
     */
    ...
    public View onCreateView(final LayoutInflater inflater,
        ...
        accountSummaryList = (LinearLayout) view
                .findViewById(R.id.summary_list);

        /** Button used to make a payment */
        final Button makePayment = (Button) view
                .findViewById(R.id.make_payament);
        makePayment.setOnClickListener(new OnClickListener() {

            @Override
            public void onClick(final View v) {
                // TODO: Implement this listener when the payment story is
                // worked on
            }
        });

        /** Hyperlink used to provide feedback */
        final TextView feedback = (TextView) view
        ...
        info = CardSessionContext.getCurrentSessionDetails()
                .getAccountDetails();
    ...
    public void populateList() {
        ...
        if (null != info.paymentDueDate) {
            final String minPaymentString = res
                    .getString(R.string.account_summary_minimum_payment);
            accountSummaryList.addView(SimpleListItemFactory.createItem(
                    context, String.format(minPaymentString,
                            getDateString(info.paymentDueDate)),
                    convertToDollars(info.minimumPaymentDue), res
                            .getString(R.string.account_summary_learn_more),
                    getPaymentActionHandler()));
        }

        final String lastPaymentString = res
                .getString(R.string.account_summary_last_payment);
        accountSummaryList.addView(SimpleListItemFactory.createItem(context,
                String.format(lastPaymentString,
                        getDateString(info.lastPaymentDate)),
                convertToDollars(info.lastPaymentAmount)));
        ...
        if (NO_BALANCE.equals(info.statementBalance)) {
            return;
        }
    ...
    protected void getLatePaymentInformation() {
        ...
        if (null == details) {
            final AsyncCallback<LatePaymentWarningDetail> callback = GenericAsyncCallback
                    .<LatePaymentWarningDetail> builder(this.getActivity())
                    .withSuccessListener(
                            new LatePaymentWarningSuccessListener(this))
                    .withErrorResponseHandler(new LatePaymentErrorHandler(this))
                    .withExceptionFailureHandler(
                            new BaseExceptionFailureHandler()).build();
    ...
    protected void getLatePaymentTextInformation() {
        // If this call has already been made, don't make it again
        if (null == text) {
            final AsyncCallback<LatePaymentWarningTextDetail> callback = GenericAsyncCallback
                    .<LatePaymentWarningTextDetail> builder(this.getActivity())
                    .withSuccessListener(
                            new LatePaymentWarningTextSuccessListener(this))
                    .withErrorResponseHandler(new LatePaymentErrorHandler(this))
                    .withExceptionFailureHandler(
                            new BaseExceptionFailureHandler()).build();
     ...
     * 
     * @param success
     *            - true if the calls were successful
     */
    ...
    /**
     * Get the fully populated content string. Include replacing the variable
     * text
     * 
    ...
    private String getContentString() {
        ...
        if (null != content) {
            content = content
                    .replace(LatePaymentWarningTextDetail.START, EMPTY);
            content = content.replace(
                    LatePaymentWarningTextDetail.PURCH_PENALTY_APR,
                    details.merchantAPR
                            + LatePaymentWarningTextDetail.VARIABLE_END);

            content = content.replace(LatePaymentWarningTextDetail.PENALTY,
                    details.aprTitle);
            content = content.replace(LatePaymentWarningTextDetail.LATE_FEE,
                    details.lateFee);
        }
        return content;
    }
