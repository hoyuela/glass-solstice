...
import com.discover.mobile.card.fastcheck.CardArtHelper.CardArtHelperCallback;
...
public class FastcheckFragment extends BaseFragment implements
    ...
    private static final long explicitRefreshInterval = 10L * 1000L; // 10
                                                                     // seconds
    ...
    public View onCreateView(final LayoutInflater inflater,
        ...
        view = (ScrollView) inflater.inflate(R.layout.fastcheck_fragment, null);

        ((NavigationRootActivity) getActivity()).getSlidingMenu()
                .setOnOpenListener(this);
        ((NavigationRootActivity) getActivity()).getSlidingMenu()
                .setOnCloseListener(this);
        cardArtHelper = new CardArtHelper(getActivity(),
                new CardArtHelperCallback() {
                    @Override
    ...
    private void getFastcheckData(boolean spinOnNoFetch) {
        ...
        if (!timeToMakeAnotherCall(context, res)) {
            if (spinOnNoFetch) {
                Utils.showSpinner(getActivity(),
                        res.getString(R.string.fast_check_spinner_msg_part1),
        ...
        String encryptedDeviceToken = FastcheckUtil
    ...
    private String formatTimeStamp() {
        SimpleDateFormat sdf = new SimpleDateFormat(TIME_ONLY_DISPLAY_FORMAT,
                Locale.US);
    ...
    private SimpleListItem createFastcheckTitleListItem(final Context context) {
        ...
        item.setCardArtImage(cardArtHelper.getCardArtFromCache());
        item.hideAction();
    ...
    private void showFastcheckErrorPage(String errorMsg, boolean processLink) {
        ...
        if (processLink) {
            ...
            fastcheckErrorMsg.setMovementMethod(LinkMovementMethod
                    .getInstance());
        } else {
    ...
    private void updateCacheAndTimestamp(FastcheckDetail aFastcheckDetail,
        ...
        cardShareDataStoreObj.addToAppCache(
                res.getString(R.string.fast_check_detail_databean),
                fastcheckDetail);
        lastUpdateTimeCal = Calendar.getInstance();
        Long tmpLastUpdateTimeInMS = Long.valueOf(lastUpdateTimeCal
                .getTimeInMillis());
        cardShareDataStoreObj.addToAppCache(
                res.getString(R.string.fast_check_last_retrieval_time_in_ms),
                tmpLastUpdateTimeInMS);
        fastcheckMaintenanceMsg = aMsg;
        cardShareDataStoreObj.addToAppCache(
                res.getString(R.string.fast_check_maintenance_msg),
                fastcheckMaintenanceMsg);
    ...
    public void onSuccess(Object data) {
        ...
        cardArtHelper.updateCardImage(((FastcheckDetail) data).cardImage,
                ((FastcheckDetail) data).incentiveTypeCode,
    ...
    private void showPreviousPage(Resources res) {
        ...
        case CANNOT_ACCESS_RESULT_PAGE:
            showFastcheckErrorPage(
                    res.getString(R.string.fast_check_error_cannot_access),
                    true);
            break;
    ...
    public void OnError(Object data) {
        ...
        if (cardErrorBean != null && Log.isLoggable(TAG, Log.ERROR)) {
            Log.e(TAG,
                    "onError() error code is " + cardErrorBean.getErrorCode());
            Log.e(TAG,
                    "onError() error msg is " + cardErrorBean.getErrorMessage());
        }
        ...
        if (cardErrorBean == null || cardErrorBean.getErrorCode() == null) {
            ...
            updateCacheAndTimestamp(null, null);
            showFastcheckErrorPage(res
                    .getString(R.string.fast_check_error_no_token));
