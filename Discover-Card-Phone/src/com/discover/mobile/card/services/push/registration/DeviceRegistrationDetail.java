 ...
 * 
 * Enum - U means the Xtify ID aka Vid is present on the server and has been
 * updated. Enum - C means the Xtify ID aka Vid was not on the server and has
 * been created.
 * 
 * @author jthornton
 * 
...
public class DeviceRegistrationDetail implements Serializable {
    ...
    /**
     * Android operating system, set a default of 4.0 in case it can't be
     * retrieved
     */
    ...
    public static enum DeviceRegStatus {
        ...
        public static DeviceRegStatus fromJsonResultCode(final String resultCode) {
            ...
            for (final DeviceRegStatus status : values()) {
                if (status.RESULT_CODE == resultCodeChar) {
                    return status;
                }
            }
            throw new RuntimeException(
                    "Unkown result status code for the device reg status.  Code: " + resultCode); //$NON-NLS-1$
        }
    }
}
