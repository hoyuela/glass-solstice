...
public class HeaderProgressIndicator extends RelativeLayout {
     ...
     * 
     * @param position
     *            - Current position between 0-2
     */
    public void initChangePasswordHeader(final int position) {
        inflateHeader();
        setTitle(R.string.enter_info, R.string.create_password,
                R.string.confirm);
    ...
    public void setPosition(final int position) {
        if (position >= 1) {
            // If the layout for step 1 is visible then set the confirm image to
            // visible
        ...
        if (position >= 2) {
            // If the layout for step 2 is visible then set the confirm image to
            // visible
    ...
    private void setIndicatorVisibility(final int position) {
        if (position == 0) {
            step1.setTextAppearance(getContext(),
                    R.style.selected_status_indicator_text);
            indicator1.setVisibility(View.VISIBLE);
            indicator2.setVisibility(View.INVISIBLE);
            indicator3.setVisibility(View.INVISIBLE);
        } else if (position == 1) {
            step2.setTextAppearance(getContext(),
                    R.style.selected_status_indicator_text);
            ...
            step3.setTextAppearance(getContext(),
                    R.style.selected_status_indicator_text);
