...
/**
 * Help adapter used to show items in the search widget. This adapter will used
 * a list of HelpItemGenerator objects to build views from the
 * help_list_item.xml file and display them in the list view of the help search
 * bar.
 * 
 * @author jthornton
 * 
...
public class HelpAdapter extends ArrayAdapter<List<HelpItemGenerator>> {
     ...
     *            - resource id that needs to be shown
     * @param data
     *            - data to be shown in the adapter (cannot be null)
     */
    public HelpAdapter(final Context context, final int textViewResourceId,
            final List<HelpItemGenerator> data) {
    ...
    /**
     * Create the view. This includes setting the correct background and
     * attaching the click listener to the view.
     * 
     * @param postion
     *            - current position
     * @param view
    ...
    public View getView(final int position, View view, final ViewGroup parent) {
        ...
        view.setBackgroundDrawable(this.getContext().getResources()
                .getDrawable(getDrawable(detail.isDark(), position)));
     ...
     * 
     * @param isDark
     *            - if the resource should be dark
     * @param position
     *            - position of the view
     */
    public int getDrawable(final boolean isDark, final int position) {
        if (data.size() == 1) {
            return (isDark) ? R.drawable.help_dark_single
                    : R.drawable.help_light_single;
        } else if (position == 0) {
            return (isDark) ? R.drawable.help_dark_top
                    : R.drawable.help_light_top;
        } else if (position == (data.size() - 1)) {
            return (isDark) ? R.drawable.help_dark_bottom
                    : R.drawable.help_light_bottom;
        } else {
            return (isDark) ? R.drawable.help_dark_middle
                    : R.drawable.help_light_middle;
        }
    }
