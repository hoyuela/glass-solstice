 ...
 * an AccountNumber field. Its functionality is toggled by using the
 * setFieldAccountNumber or setFieldUsername methods.
 * 
 * This class contains functionality for both types of input fields because I
 * could not find a better solution for handling the input field changing
 * between registration and forgot password.
 * 
 * @author scottseward
 * 
 */
public class UsernameOrAccountNumberEditText extends ValidatedInputField {
    ...
    /**
     * Default constructors. Initially sets up the input field as a username
     * field.
     */
    ...
    /**
     * Change the input field to accept and validate a username. This is an
     * alphanum input with no suggestions and a max length of 32 characters.
     */
    ...
    /**
     * Change the input field to accept and validate an account number. This
     * means it only accepts numbers up to the max account number length.
     */
    ...
    /**
     * Listens for hardware keyboard inputs and stylizes the input for account
     * numbers.
     * 
     * When text is changed in the input field it has to do the following. Save
     * the current input that has just changed and get the properly formatted
     * version of the current input. Compare the current input to a properly
     * formatted version of the current input. If they differ in formatting,
     * restore the input field with the properly formatted String. Then restore
     * the position of the cursor.
     * 
     * This also needs to know when a deletion is being made, by comparing
     * before and after lengths of the String. Also it needs to know where
     * spaces are being inserted so that it can account for cursor position.
     */
    private void setupInputStylizer() {
        this.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(final CharSequence s, final int start,
                ...
                String currentTextStylized = CommonUtils
                        .getStringWithSpacesEvery4Characters(CommonUtils
                                .getSpacelessString(currentText));
                ...
                if (currentTextStylized.length() <= VALID_ACCOUNT_NUMBER_LENGTH
                        && !currentText.equals(currentTextStylized)) {
                    saveCursorPosition();
    ...
    private void restoreCursorPosition() {
        if (needsToRestore) {
            ...
            // increment the cursor position so that it restores to the right
            // spot
            // ahead of the space.
    ...
    /**
     * Sets the max length of the input to the maximum username length. Also
     * changes the input type to be alphanum
     */
    ...
    /**
     * This method validates the current input for the set input field type. If
     * the input is valid, it returns true, otherwise false.
     */
    @Override
    ...
    /**
     * Returns true if the username is valid, in this case, if it is not empty
     * or null.
     * 
    ...
    public boolean isUsernameValid() {
        ...
        return InputValidator.valueBoundedBy(usernameLength,
                MIN_USERNAME_LENGTH, MAX_USERNAME_LENGTH);
    }

    /**
     * Checks to see if the input looks like a valid account number. If it
     * begins with 6011 and is of length 16.
     * 
