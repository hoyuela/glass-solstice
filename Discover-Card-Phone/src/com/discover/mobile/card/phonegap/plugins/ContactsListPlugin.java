...
public class ContactsListPlugin extends CordovaPlugin {
    ...
    @TargetApi(Build.VERSION_CODES.HONEYCOMB)
    String queryContentUriWork(Uri contactData, ArrayList<String> numbers,
            ArrayList<String> emails, String name) {
        ...
        if (Build.VERSION.SDK_INT >= 11) {
            ...
            c = this.cordova.getActivity().managedQuery(contactData, null,
                    null, null, null); // manageQuery self-releases object
        ...
        case PICK_CONTACT:
            if (resultCode == Activity.RESULT_OK) {
                ...
                if (numbers.size() > 0 || emails.size() > 0) {
                    ...
                    this.cordova
                            .setActivityResultCallback(ContactsListPlugin.this);
                    this.cordova.getActivity().startActivityForResult(intent,
                            PICK_CREDENTIAL);
                } else {
                    Uri contactsContractUri = ContactsContract.Contacts.CONTENT_URI;
