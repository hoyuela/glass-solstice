 ...
 * @version 1.0
 * 
 *          This class serves as a broker to download a File via HTTP/S and save
 *          it to a given location on an Android device.
 * 
 */
public class ResourceDownloader {
    ...
    /**
     * This method will download a file from the Web and Save it in the
     * directory specified, as well as return a handle to the file.
     * 
     * @param urlStr
     *            - URL where resource can be downloaded from
     * @param method
     ...
     *            - HTTP Headers
     * @param directory
     *            - Full Directory Name to where the File should be saved
     * @param fileExt
     *            - File extension, eg: .pdf
     * @param contentType
     *            - The Content Type - if not in the response, exception is
     *            thrown
     * 
     * @return
     * @throws Exception
     */
    public File executeDownload(final String urlStr, final String method,
            final String inputContent, final HashMap<String, String> headers,
            final String directory, final String fileExt,
            final String contentType) throws Exception {
        HttpURLConnection connection = null;
        int statusCode = DEFAULT_STATUS_CODE;
        File theFile = null;
        try {
            final boolean isSecure = urlStr.indexOf(HTTPS) >= 0;
            final URL url = new URL(urlStr);
            if (isSecure) {
                connection = (HttpsURLConnection) url.openConnection();
                final SSLContext context = SSLContext.getInstance(TLS);
                context.init(null, null, null);
                final SSLSocketFactory sockFactory = context.getSocketFactory();
                ((HttpsURLConnection) connection)
                        .setSSLSocketFactory(sockFactory);
            } else {
            ...
            if (GET.equals(method)) {
                ...
                try {
                    if (inputContent != null) {
                        ...
                        Utils.log(LOG_TAG, "Setting inputContent: "
                                + inputContent);
            ...
            final String respMsg = connection.getResponseMessage();
            Utils.log(LOG_TAG, "HTTP Response: " + statusCode + " " + respMsg);

            if (statusCode != 200) {
                throw new Exception("Bad Return. HTTP Response: " + statusCode
                        + " " + respMsg);
            }

            // Check to see if a filename was given in the response
            // eg Content-Disposition: attachment;
            // filename=Discover-Statement-20120520.pdf
            boolean contentTypeFound = false;
            String respFileName = null;
            if (connection.getHeaderFields() != null) {
                for (final String header : connection.getHeaderFields()
                        .keySet()) {
                    if (HDR_CNT_DSP.equalsIgnoreCase(header)) {
                        for (final String headerVal : connection
                                .getHeaderFields().get(header)) {
                            final int fnIdx = headerVal.indexOf(FILE_NAME);
                            if (fnIdx >= 0) {
                                respFileName = headerVal.substring(fnIdx
                                        + FILE_NAME.length());
                            }
                        }
            ...
            final StringBuilder buf = new StringBuilder(directory);
            buf.append(FS);
            if (respFileName != null) {
                ...
                if (urlStr.indexOf(fileExt) > 0) {
                    buf.append(urlStr.substring(urlStr.lastIndexOf(FS) + 1,
                            urlStr.indexOf(fileExt)));
                } else {
                    // File extension does not appear in the URL so lets just
                    // make the name up.
            ...
            try {
                ...
                throw new Exception(
                        "Exception caught trying to fetch the response content: "
                                + e.getMessage());
            }
        } catch (final Exception e) {
            throw new Exception("Exception caught sending request: "
                    + e.getMessage());
        } finally {
            if (connection != null) {
