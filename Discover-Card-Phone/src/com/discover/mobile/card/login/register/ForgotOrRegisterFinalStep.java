...
public class ForgotOrRegisterFinalStep extends NotLoggedInRoboActivity
    ...
    protected void retrieveAccountDetailsFromServer(
        ...
        final CardEventListener cardEventListener = new CardEventListener() {
            ...
            public void onSuccess(final Object data) {
                ...
                PageTimeOutUtil.getInstance(ForgotOrRegisterFinalStep.this)
                        .destroyTimer(); // DEFECT 96936
     ...
     */
    /*
     * protected void getXtifyRegistrationStatus() { if
     * (XtifySDK.getXidKey(this) != null) { final
     * AsyncCallback<PushRegistrationStatusDetail> callback =
     * GenericAsyncCallback .<PushRegistrationStatusDetail> builder(this)
     * .showProgressDialog( getResources().getString(
     * R.string.push_progress_get_title), getResources() .getString(
     * R.string.push_progress_registration_loading), true)
     * .withSuccessListener(new PushConfirmationSuccessListener())
     * .withErrorResponseHandler( new PushRegistrationStatusErrorHandler(
     * FacadeFactory.getLoginFacade() .getLoginActivity()))
     * .withExceptionFailureHandler( new BaseExceptionFailureHandler())
     * 
     * .finishCurrentActivityOnSuccess(this).build();
     * 
     * new GetPushRegistrationStatus(this, callback).submit(); } else { // If
     * the device does not have an XidKey, navigate to the next // screen
     * anyway.
     * navigateToConfirmationScreenWithResponseData(confirmationDetails);
     * finish(); }
     * 
    ...
    /*    *//**
     * The original success listener needed to be extended to support
     * navigating to another screen on success. This specific class handles
     * navigating to the home screen after a user completes registration and the
     * push notification status is retrieved.
     * 
     * @author scottseward
     * 
     */
     ...
     *//**
     * Constructor that takes in a context so that it can manipulate the flow
     * of the app.
     */
    /*
     ...
     */
    /*
     * @Override public void success(final NetworkServiceCall<?> sender, final
     * PushRegistrationStatusDetail value) { super.success(sender, value);
     * navigateToConfirmationScreenWithResponseData(confirmationDetails); } }
     */
    ...
    public void OnError(final Object data) {
         ...
         * true); bundle.putBoolean(IntentExtraKey.SESSION_EXPIRED, true);
