...
public class HomeSummaryFragment extends BaseFragment implements
    ...
    public View onCreateView(final LayoutInflater inflater,
        ...
        boolean otherUser = sharedPreferences.getBoolean(
        ...
        if (bundle != null) {
            String deeplink = bundle.getString(
                    CardNavigationMenuFragment.DEEPLINK, null);
            if (deeplink != null
                    && deeplink
                            .equals(CardNavigationMenuFragment.DEEPLINK_PASSCODE)) {
                isDeeplinkMode = true;
        ...
        if (null != accountDetails) {
            if (null != accountDetails.mailingAddress) {
                if (null != accountDetails.mailingAddress.firstName) {
                    String lowerCaseName = accountDetails.mailingAddress.firstName
                            .toLowerCase();
                    String formatName = lowerCaseName.substring(0, 1)
                            .toUpperCase() + lowerCaseName.substring(1);
                    defaultHiText.append(" " + formatName);
                }
            } else {

                if (null != accountDetails.primaryCardMember) {
                    if (null != accountDetails.primaryCardMember.nameOnCard) {
                        String lowerCaseName = accountDetails.primaryCardMember.nameOnCard
                                .toLowerCase();
                        String formatName = lowerCaseName.substring(0, 1)
                                .toUpperCase() + lowerCaseName.substring(1);
                        defaultHiText.append(formatName);
    ...
    /**
     * To fetch the Salutation Message
     */
                ...
                public void onGlobalLayout() {
                    if (!toggleView.hasIndicatorBeenDrawn()) {
                        toggleView.setIndicatorPosition(
                                cardBankToggle.getLeft() + 7,
                                cardBankToggle.getTop() + 11,
