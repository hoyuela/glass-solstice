	...
	submitSearchForm = function(sortOrder) {
		if(searchSubmitValidation()){
			...
			if(dateOption === "specific"){
				...
				searchOptions += "&dateType=one&fromDate="+fromDate;
				dateOption = "On " +fromDate;
			}else if(dateOption === "range"){
				var fromDate = $("#fromDate").val();
				var toDate = $("#toDate").val();
				searchOptions += "&dateType=range&fromDate="+fromDate+"&toDate="+toDate;
			...
			if(amountOption === "specific"){
				...
				searchOptions += "&amountType=one&fromAmount="+fromAmount;
				amountOption = " $"+fromAmount;
			}else if(amountOption === "range"){
				var fromAmount = $("#fromAmount").val();
				var toAmount = $("#toAmount").val();
				searchOptions += "&amountType=range&fromAmount="+fromAmount+"&toAmount="+toAmount;
			...
			var displaySearchResults = function(searchResultData){
				...
				if(totalTransactions === 0){
					searchResults += "<p><span class=\"inlineLabel boldtext\">We did not find any matching transactions.</span></p>";
					searchResults += "<p><span class=\"inlineLabel boldtext\">Please allow 1-3 days for recent transactions to post.</span></p>";
					searchResults += "</div>";	
				}else{
