...
dfs.crd.achome.populatePushCountDivs = function()
{
    ...
    if (dfs.crd.lilo.pushNewMsgCount == 0)
    {
        ...
        if (dfs.crd.lilo.registrationServiceCalled && !dfs.crd.lilo.otherUser && getClientPlat()=="iPhone")
        {
    ...
    else
    {
        ...
        $(".notification_image_header").removeClass("notification_image_header").addClass("notification-header-icon");
...
dfs.crd.achome.getACHomeData = function(pageId)
{
	try
	{
		...
		{
			var newDate = new Date();
			var ACHOMEURL = RESTURL+"acct/v1/account?"+newDate+"";      
            showSpinner();
			$.ajax
			({    
				type : "GET",
