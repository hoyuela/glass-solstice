	...
	dfs.crd.rwd.availablePartners = HREF_URL + "mobile/rewards/advanced/images/partners/available/";
	dfs.crd.rwd.unavailablePartners = HREF_URL + "mobile/rewards/advanced/images/partners/unavailable/";
	...
	try {
			console.log("********loadredemptionLanding********");
			...
			if (!jQuery.isEmptyObject(allPartersDetails)) {									
				...
				$("#ecertsAndGiftCardsCount").html(totalPartnersDisplay+"("+totalPartnersCount+")");
			}else{
	...
	/***************** AJAX CALL TO GET ALL THE PARTNERS TO STORE IN THE GLOBAL CACHE*****************/
	...
	dfs.crd.rwd.getAllPartners = function(){
		...
		try {
			...
			var ALLECERTSPARTNERSURL = RESTURL + "rewards/v2/partners?rewardType=ALL&"
			+ newData + "";	
			dfs.crd.rwd.insuficientErrorPresent = 'false';
	...
	function browseLandingLoad(){
		...
		try {
			showSpinner();
			killDataFromCache("REDEEMPARTNERDATA"); //kill any partner data if present if user click back to browse landing to select another merchant
			killDataFromCache("REDEEMSELCETED");
			/**************************************************************/
			...
			if (!jQuery.isEmptyObject(cachedAllPartnerData)) {	
				...
				$("#allpartners").html(browseAllPartnersStr+"("+totalPartnersCount+")");	
				$("#ecerts").html(browseEcertsStr+"("+totalEcertsCount+")");	
				$("#partnerGC").html(browsegiftCardsStr+"("+totalPartnerGCCount+")");	
			}
			console.log("totalPartnersCount:: " + totalPartnersCount + "::totalEcertsCount::"+ totalEcertsCount);
	...
	dfs.crd.rwd.browseCategory = function(categoryCode){
		...
		try {
			...
			/**************************************************************/
			var partnerCategoryDetails = dfs.crd.rwd.getPartnerCategoryData(categoryCode);
	...
	dfs.crd.rwd.getPartnerCategoryData = function(categoryCode){
		...
		console.log("getPartnerCategoryData :: " + categoryCode);
			var newData = new Date();
			var getCategoryDataURL=RESTURL+"rewards/v2/partners?rewardType=ALL&category="+categoryCode;
		...
		try {
			...
			if (!jQuery.isEmptyObject(cachedCategoryData)) {	
				...
				if (!isEmpty(partnerCatgList) && partnerCatgList.length !==0)	{	
					...
					/*For handset only one category data will be presented . So there will be only one category in the ctgyCounts list*/					
					...
					console.log("empty check");
						...
						console.log("catgCount ::" + catgCount + ":: catgCd::"+ catgCd);
						$("#categoryCount").html(catgCount);	
						$("#categoryDesc").html(catgDs + " " + "<span>("+catgCount+")</span>");	
					
					}
					
					var partnerCatgListUL = "<ul class='partners' data-filter-placeholder='Search "+catgDs+" Partners' data-icon='search' data-role='listview' data-filter='true' data-theme='d' data-inset='true' data-filter-theme='d' data-icon='search'>";
					
					$.each (partnerCatgList, function (category) {	
						...
						var catgUnavailableStr = "<p> You're only $"+ctgmissingAmt+" <i>Cashback Bonus</i> away from being able to get $"+ctgminRedemptionAmount+" for $"+ctgminWhatYouGetAmount+".</p>";
						var catgpg = "fromALL";
						var activeCatgLi = "";	
						if(partnerCatgList[category].isPartnerAvail === true){ // for available partners
							
								activeCatgLi += "<li data-filtertext='"+partnerCatgModeDesc+"'>";
								activeCatgLi += "<a href='#' onClick=dfs.crd.rwd.renderPartnerDetails('"+ partnerCatgModeCode + "','"+ catgpg + "')>";
								activeCatgLi += "<img width='104' height='56' src='"+ dfs.crd.rwd.availablePartners + ""+ partnerCatgModeCode + ".png'";
								activeCatgLi += "><h3>"+partnerCatgModeDesc+"</h3></a>";
								activeCatgLi += "</li>";
								partnerCatgListUL += activeCatgLi;
							
							}else{ //for Unavailable partners
								activeCatgLi += "<li class='disabled-partnerlist' data-filtertext='"+partnerCatgModeDesc+"'>";
								activeCatgLi += "<a href='#'>";
								activeCatgLi += "<img width='104' height='56' src='"+ dfs.crd.rwd.unavailablePartners + ""+ partnerCatgModeCode + ".png'";
								activeCatgLi += "><h3>"+partnerCatgModeDesc+"</h3>";
	...
	function browseAllPartnersLoad(){
		try {
			...
			if (!jQuery.isEmptyObject(cachedallPartnerData)) {	
				...
				if (allPartnersList !== null && allPartnersList !== 'undefined' && allPartnersList !== '' && allPartnersList.length !==0)	{					
					$.each (allPartnersList, function (partner) {		
							...
							var unavailableStr = "<p> You're only $"+missingAmt+" <i>Cashback Bonus</i> away from being able to get $"+minRedemptionAmount+" for $"+minWhatYouGetAmount+".</p>";
							//var unavailableStr = "<p >You don't have enough <i><i>Cashback Bonus</i></i> to redeem at this merchant.Keep earning!</p>";
							var pg = "fromALL";
							var activePartnerLi = "";						
							if(allPartnersList[partner].isPartnerAvail === true){ // for available partners
							
								activePartnerLi += "<li data-filtertext='"+partnerModeDesc+"'>";
								activePartnerLi += "<a href='#' onClick=dfs.crd.rwd.renderPartnerDetails('"+ partnerModeCode + "','"+ pg + "')>";
								activePartnerLi += "<img width='104' height='56' src='"+ dfs.crd.rwd.availablePartners + ""+ partnerModeCode + ".png'";
								activePartnerLi += "><h3>"+partnerModeDesc+"</h3></a>";
								activePartnerLi += "</li>";
								allPartnersULList += activePartnerLi;
							
							}else{ //for Unavailable partners
								activePartnerLi += "<li class='disabled-partnerlist' data-filtertext='"+partnerModeDesc+"'>";
								activePartnerLi += "<a href='#'>";
								activePartnerLi += "<img width='104' height='56' src='"+ dfs.crd.rwd.unavailablePartners + ""+ partnerModeCode + ".png'";
								activePartnerLi += "><h3>"+partnerModeDesc+"</h3>";
				...
				if (typeof allPartnersList !== 'undefined') {
					var partnerListLen = allPartnersList.length;
					console.log(partnerListLen);
					for(i=0;i<partnerListLen;i++){
					
						var partnerData = allPartnersList[i];
						var partnerModeCode = partnerData.modeCode;
						var partnerModeDesc = partnerData.modeDesc;						
						var missingAmt = partnerData.amtAwayToRedeem;
						var minRedemptionAmount = partnerData.minModeAmt;
						var minWhatYouGetAmount = partnerData.minDisbAmt;
						var unavailableStr = "<p> You're only "+missingAmt+" <i>Cashback Bonus</i> away from being able to get "+minRedemptionAmount+" for "+minWhatYouGetAmount+".</p>";
						
						var activePartnerLi = "";						
						if(partnerData.isPartnerAvail === true){ // for available partners
						
							activePartnerLi += "<li>";
							activePartnerLi += "<a href='#' onClick=renderRedeemPartner1('"+ partnerModeCode + "')>";
							activePartnerLi += "<img width='104' height='56' src='"+ "" + ""+ partnerModeCode + ".png'";
							activePartnerLi += "><h3>"+partnerModeDesc+"</h3></a>";
							activePartnerLi += "</li>";
							allPartnersULList += activePartnerLi;
						
						}else{ //for Unavailable partners
							activePartnerLi += "<li class='disabled-partnerlist'>";
							activePartnerLi += "<a href='#'>";
							activePartnerLi += "<img width='104' height='56' src='"+ "" + ""+ partnerModeCode + ".png'";
							activePartnerLi += "><h3>"+partnerModeDesc+"</h3>";
							activePartnerLi += unavailableStr;
			...
			if (!jQuery.isEmptyObject(allPartnersData)) {	
				...
				if (allpartnersList !== null && allpartnersList !== 'undefined' && allpartnersList !== '' && allpartnersList.length !== 0)	{					
						$.each (allpartnersList, function (partner) {		
								...
								var unavailableEcertStr = "<p> You're only $"+emissingAmt+" <i>Cashback Bonus</i> away from being able to get $"+eminRedemptionAmount+" for $"+eminWhatYouGetAmount+".</p>";
								//var unavailableEcertStr = "<p >You don't have enough <i><i>Cashback Bonus</i></i> to redeem at this merchant.Keep earning!</p>";
								var browseEcertLi = "";						
								if(allpartnersList[partner].hasECert === true){ // for available partners
								
									if(allpartnersList[partner].isPartnerAvail === true){
										browseEcertLi += "<li data-filtertext='"+epartnerModeDesc+"'>";
										browseEcertLi += "<a href='#' onClick=dfs.crd.rwd.renderPartnerDetails('"+ epartnerModeCode + "','"+ pgECT + "')>";
										browseEcertLi += "<img width='104' height='56' src='"+ dfs.crd.rwd.availablePartners + ""+ epartnerModeCode + ".png'";
										browseEcertLi += "><h3>"+epartnerModeDesc+"</h3></a>";
										browseEcertLi += "</li>";
										allEcertsULList += browseEcertLi;
										availEcertsCount = availEcertsCount + 1;
									}else{
										browseEcertLi += "<li class='disabled-partnerlist' data-filtertext='"+epartnerModeDesc+"'>";
										browseEcertLi += "<a href='#' onClick=renderRedeemPartner1('"+ epartnerModeCode + "')>";
										browseEcertLi += "<img width='104' height='56' src='"+ dfs.crd.rwd.unavailablePartners + ""+ epartnerModeCode + ".png'";
										browseEcertLi += "><h3>"+epartnerModeDesc+"</h3>"
						...
						console.log("availEcertsCount :: "+ availEcertsCount + ":: unavailEcertsCount:: " + unavailEcertsCount); 
	...
	function browse_all_gcardLoad(){
		...
		try {
			...
			if (!jQuery.isEmptyObject(allPartnerDetails)) {	
				...
				console.log("globalEarnRewardAmount :: "+ globalEarnRewardAmount);
				...
				var pgGCD = "fromGCD";
				if (allPrtnrsList !== null && allPrtnrsList !== 'undefined' && allPrtnrsList !== '' && allPrtnrsList.length !== 0)	{					
					$.each (allPrtnrsList, function (partner) {		
							...
							var unavailableGcardtStr = "<p> You're only $"+gcmissingAmt+" <i>Cashback Bonus</i> away from being able to get $"+gcminRedemptionAmount+" for $"+gcminWhatYouGetAmount+".</p>";
							//var unavailableGcardtStr = "<p >You don't have enough <i><i>Cashback Bonus</i></i> to redeem at this merchant.Keep earning!</p>";
							var browseCardLi = "";						
							if(allPrtnrsList[partner].hasGiftCard === true){ 
							
								if(allPrtnrsList[partner].isPartnerAvail === true){// for available partners
							
									browseCardLi += "<li data-filtertext='"+gcpartnerModeDesc+"'>";
									browseCardLi += "<a href='#' onClick=dfs.crd.rwd.renderPartnerDetails('"+ gcpartnerModeCode + "','"+ pgGCD + "')>";
									browseCardLi += "<img width='104' height='56' src='"+ dfs.crd.rwd.availablePartners + ""+ gcpartnerModeCode + ".png'";
									browseCardLi += "><h3>"+gcpartnerModeDesc+"</h3></a>";
									browseCardLi += "</li>";
									...
									browseCardLi += "<li class='disabled-partnerlist' data-filtertext='"+gcpartnerModeDesc+"'>";
									browseCardLi += "<a href='#' onClick=renderRedeemPartner1('"+ gcpartnerModeCode + "')>";
									browseCardLi += "<img width='104' height='56' src='"+ dfs.crd.rwd.unavailablePartners + ""+ gcpartnerModeCode + ".png'";
									browseCardLi += "><h3>"+gcpartnerModeDesc+"</h3>";
					...
					console.log("availGcardsCount :: "+ availGcardsCount + ":: unavailGcardsCount:: " + unavailGcardsCount); 
	...
	dfs.crd.rwd.getPartnerDetails = function(modeCode){
		...
		//AJAX CALL
			...
			var newData = new Date();
			var PARTNERSURL = RESTURL + "rewards/v2/redeemoption?modeCode="+modeCode+"&" + newData + "";			
			showSpinner();
	...
	function redeemPartner1Load(){
		try {
			...
			if (!jQuery.isEmptyObject(partnerDataForRedeem)) {
				...
				if (partnerDataForRedeem.billingAddr !== null && partnerDataForRedeem.billingAddr !== 'undefined' && partnerDataForRedeem.billingAddr !== ''){				
					billingDetails = partnerDataForRedeem.billingAddr;				
				}				
				$("#pmodeDesc").html(pmodeDesc);	
				console.log("Merchant description ::::: " + partnerDataForRedeem.partnerDesc);
				$("#merchantDesc").html(partnerDataForRedeem.partnerDesc);
				
				$("#stepper1").val("1"); //set the default quantity
				
				/*If call is coming from Browse ALL,Browse all ECertificates, Best value, categories
					then ecertificates will be selected by default. If call is coming from Browse all gift cards then gift card
				...
				if (hasEcert === true && hasGiftCard === true){					
					...
					if(pageNameT === "fromALL"){//Call coming from Browse ALL
						...
						$("#ecertFloat").html("<span class='amt-btn'><a href='#' class='blk-wht-btn r5-btn ui-link' id='btn-eCertificate'><span class='chk chk-icon'></span>eCertificate</a></span>");
						$("#giftCardPresent").html("<a href='#' class='drkgry-wht-btn r5-btn ui-link' id='btn-giftCard'><span class='chk'></span>Gift Card</a>");
					}else if (pageNameT === "fromECT"){// Call coming from Browse Ecerts
						$("#redeem-dtlview").jqmData('ECT', 'true');
						$("#redeem-dtlview").jqmData('GCD', '');
						$("#ecertFloat").html("<span class='amt-btn'><a href='#' class='blk-wht-btn r5-btn ui-link' id='btn-eCertificate'><span class='chk chk-icon'></span>eCertificate</a></span>");
						$("#giftCardPresent").html("<a href='#' class='drkgry-wht-btn r5-btn ui-link' id='btn-giftCard'><span class='chk'></span>Gift Card</a>");
						
					}else if(pageNameT === "fromGCD"){// Call coming from Browse Gcards
						$("#redeem-dtlview").jqmData('GCD', 'true');
						$("#redeem-dtlview").jqmData('ECT', '');
						$("#ecertFloat").html("<span class='amt-btn'><a href='#' class='drkgry-wht-btn r5-btn ui-link' id='btn-eCertificate'><span class='chk'></span>eCertificate</a></span>");
						$("#giftCardPresent").html("<a href='#' class='blk-wht-btn r5-btn ui-link' id='btn-giftCard'><span class='chk chk-icon'></span>Gift Card</a>");
					...
					$("#giftCardPresent").html("<a href='#' class='blk-wht-btn r5-btn ui-link'><span class='chk chk-icon'></span>eCertificate</a>");
					$("#ecertFloat").html("");
				...
				if (disbAmts !== null && disbAmts !== 'undefined' && disbAmts !== '')	{				
					...
					defaultSpan += "<section data-amount-pay="+defaultModeAmt+" data-amount-get="+defaultDisbAmt+" data-max-qty="+defaultQuantity+">";
					defaultSpan += "<span class='amt-btn'> <a href='#' id='default' class='blk-wht-btn r5-btn ui-link'> <span class='chk chk-icon'></span><span class='yellow-bg'> <span class='white-doller'>$"+defaultModeAmt+"</span> <span class='yellow-doller'>$"+defaultDisbAmt+"</span> </span> </a> </span>"
					defaultSpan += "</section>"
					...
					for(i=1;i<amountCount;i++){						
						...
						if(isAvailToRedeem === true){							
							amtSpan += "<section data-amount-pay="+modeAmt+" data-amount-get="+disbAmt+" data-max-qty="+maxQuantity+">";
							amtSpan += "<span class='amt-btn'><a href='#' id='"+modeAmt+"' class='drkgry-wht-btn r5-btn ui-link'> <span class='chk'></span><span class='yellow-bg'> <span class='white-doller'>$"+modeAmt+"</span> <span class='yellow-doller'>$"+disbAmt+"</span> </span></a></span>";
							amtSpan += "</section>";
						
						}else{							
							amtSpan += "<section data-amount-pay='' data-amount-get='' data-max-qty=''>";
							amtSpan += "<span><a href='#' class='drkgry-wht-btn r5-btn disabled-btn ui-link'> <span class='chk'></span><span class='disabled-bg'> <span class='white-doller'>$"+modeAmt+"</span> <span class='yellow-doller'>$"+disbAmt+"</span> </span> </a></span>"
							amtSpan += "</section>";
						}				
					}
				...
				$("#partnerImg").attr('src',(dfs.crd.rwd.availablePartners + "" + pmodeCode + ".png"));
				...
				if (!jQuery.isEmptyObject(trySelected)) {					
					...
					console.log("trySelected.selectedQuantity" + trySelected.selectedQuantity);
					
										
					$("#stepper1").val(trySelected.selectedQuantity);
					/*if(trySelected.isGCD === true && trySelected.ECT === false){
						console.log("only gift card");
						$("#ecertFloat").html("<span class='amt-btn'><a href='#' class='drkgry-wht-btn r5-btn ui-link' id='btn-eCertificate'><span class='chk'></span>eCertificate</a></span>");
						$("#giftCardPresent").html("<a href='#' class='blk-wht-btn r5-btn ui-link' id='btn-giftCard'><span class='chk chk-icon'></span>Gift Card</a>");
			
					}else if(trySelected.isGCD === false && trySelected.ECT === true){
						console.log("only ECT");
						$("#giftCardPresent").html("<a href='#' class='blk-wht-btn r5-btn ui-link'><span class='chk chk-icon'></span>eCertificate</a>");
						$("#ecertFloat").html("<p class='text ecart-txt ecart-padd'>eCertificates can be used <br />instantly online or in stores.</p>");
						
					}else */
					
					if(selGCD === 'true' && !selECT.length > 0){
					...
					/*if(selAmtID === undefined){	
						amount_selected_id = "#"+ amtIdD;	
						console.log(" amount_selected_id when undefined" + amount_selected_id);								
						$(amount_selected_id).removeClass("drkgry-wht-btn").addClass("blk-wht-btn").find("span.chk").addClass("chk-icon");
						$("#default").removeClass("blk-wht-btn").addClass("drkgry-wht-btn").find("span.chk").removeClass("chk-icon");							
					}*/
				...
				/********************* handle insufficient error landing back to setp1 ******************************/
				
				if(dfs.crd.rwd.insuficientErrorPresentSubmit === 'true'){
					//redemption took place from some other channel. Have to update the global CBB
					killDataFromCache("ACHOME");
					globalEarnRewardAmount = numberWithCommas(errAvailCBB);
					$("#redeemPartner1_CBB").html("$"+ errAvailCBB);	
					console.log("error after submit" + errAvailCBB + "globalEarnRewardAmount :: " + globalEarnRewardAmount);
					$("#inSuffError").show();
				...
				/**************************************************CLICK CONTINUE OR TERMS*********************************************/
				
				
				$("#common-continue-btn").click(function(){	
													
					var selectedModeAmount = $('#redeem-dtlview').jqmData('redeemAmountPay');
					var selectedDisbAMount = $('#redeem-dtlview').jqmData('redeemAmountGet');
					var maxQuantity = $('#redeem-dtlview').jqmData('redeemMaxQty');
					var selectedQuantity = $("#stepper1").val(); 
					var GCD = $('#redeem-dtlview').jqmData('GCD');
					var ECT = $('#redeem-dtlview').jqmData('ECT');
					var getSelectedAmtId = $("#redeem-dtlview").jqmData('selectedAmtId');
					var alreadyCheckedAmount = $("#pills").find("a.blk-wht-btn").attr("id");
					console.log("getSelectedAmtId ::: " + getSelectedAmtId + ":: alreadyCheckedAmount :;" + alreadyCheckedAmount + "::selectedQuantity::"+ selectedQuantity);
					
					var selectedRedemptionDetls =
							dfs.crd.rwd.createSelectedRedeemDataCache(selectedModeAmount,selectedDisbAMount,maxQuantity,selectedQuantity,GCD,ECT,pmodeCode,pmodeDesc,getSelectedAmtId,alreadyCheckedAmount);
							
					//console.log("GCD ::"+ GCD + "ECT :::" + ECT);					
					if (GCD === 'true' && GCD.length > 0){
						console.log("navigate to gift card verify page");
						if (billingDetails !== null && billingDetails !== 'undefined' && billingDetails !== ''){	
						console.log("creating billing addr");			
							var billingAddr = '';
							billingAddr += billingDetails.fullName;
							billingAddr +='<br />';
							billingAddr +=billingDetails.addrLine1;
							if(billingDetails.addrLine2 !=='' && billingDetails.addrLine2!=='undefined'){
								billingAddr +=billingDetails.addrLine2;
							}
							if(billingDetails.addrLine3 !=='' && billingDetails.addrLine3!=='undefined'){
							...
							billingAddr +=billingDetails.zipCode;
						 	...
						 	console.log("billing address string == " + billingAddr);
						...
						putDataToCache("REDEEMSELCETED", selectedRedemptionDetls);
						navigation('../rewards/redeemPartnerGCD2');
					}else if(ECT === 'true' && ECT.length > 0){
					
						console.log("navigate to ecertificate verify page");
				...
				$("#redeemPartner1_terms").click(function(){
					...
					var getSelectedAmtId = $("#redeem-dtlview").jqmData('selectedAmtId');		
					var alreadyCheckedAmount = $("#pills").find("a.blk-wht-btn").attr("id");		
					var seltdRedeemDetails =
							dfs.crd.rwd.createSelectedRedeemDataCache(selectedModeAmount,selectedDisbAMount,maxQuantity,selectedQuantity,GCD,ECT,pmodeCode,pmodeDesc,getSelectedAmtId,alreadyCheckedAmount);
					
					putDataToCache("REDEEMSELCETED", seltdRedeemDetails);
			...
			dfs.crd.rwd.insuficientErrorPresentSubmit = 'false'; //after the error page is laoded set it to false
	...
	dfs.crd.rwd.createSelectedRedeemDataCache = function(selectedModeAmount,selectedDisbAMount,maxQuantity,selectedQuantity,GCD,ECT,pmodeCode,pmodeDesc,getSelectedAmtId,alreadyCheckedAmount){
		try{
	...
	function redeemPartnerGCD2Load(){
		try{
			...
			var validPriorPagesOfRenderGCD2= new Array("redeemPartner1","redeemMerchantTerms");
			if(jQuery.inArray(fromPageName, validPriorPagesOfRenderGCD2) > -1 ){
				...
				if (!jQuery.isEmptyObject(selectedRedemptionData)) {
					...
					$("#giftCardDisbAmt").html(selectedRedemptionData.selectedDisbAMount);
					$("#gcmodeDesc").html(selectedRedemptionData.modeDesc);
					$("#giftCardModeAmt").html("$"+ (numberWithCommas(gcSelectdModeAmt)) + ".00");
					//console.log("mode amt " + selectedRedemptionData.selectedModeAmount);
					...
					console.log("selected GCD::::" + selectedRedemptionData.isGCD + " :: ECT ::" + selectedRedemptionData.ECT);
	...
	function redeemPartnerECT2Load(){
		...
		try{
			var validPriorPagesOfRenderECT2= new Array("redeemPartner1","redeemMerchantTerms");
			if(jQuery.inArray(fromPageName, validPriorPagesOfRenderECT2) > -1 ){		
				...
				if (!jQuery.isEmptyObject(selectdRedemptionData)) {
					...
					if(selectdRedemptionData.selectedQuantity > 1){
						console.log("quantity more than 1" + selectdRedemptionData.selectedQuantity);
						$("#selectdQty").html(selectdRedemptionData.selectedQuantity+" - ");	
						slctdModeAmt = slctdModeAmt * (selectdRedemptionData.selectedQuantity);
						$("#gift-card-verify-partner").find(".pluralTxt").removeClass("pluralTxt");
					...
					$("#ectCardDisbAmt").html(selectdRedemptionData.selectedDisbAMount);
					$("#ectmodeDesc").html(selectdRedemptionData.modeDesc);
					$("#ectCardModeAmt").html("$"+ (numberWithCommas(slctdModeAmt))+".00");
					
					var amt1 = globalEarnRewardAmount.replace(",", "");											
					var remainingECTCBB = "$" + ((amt1 - (slctdModeAmt)).toFixed(2));
			...
			if (!jQuery.isEmptyObject(merchantDetailsToPost)) {
				...
				console.log("updatedPostDetails no empty");
					...
					if(!isEmpty(updatedPostDetails.availToRedeem)){
						//console.log("updated CBB after redeem ::" + updatedPOSTDetails.availToRedeem); 
						globalEarnRewardAmount = numberWithCommas(updatedPostDetails.availToRedeem);
					}
					killDataFromCache("REDEEMSELCETED"); // kill the selected data from step1 now
	...
	function redeemPartnerECT3Load(){
		try {
			...
			var validPriorPagesOfECT3= new Array("redeemPartnerECT2","redeemInstructions","redeem_ecert_printphotos","redeem_ecart_savetophotos_pin","redeemMerchantSubmitTerms");
			if(jQuery.inArray(fromPageName, validPriorPagesOfECT3) > -1 ){	
				...
				if (!jQuery.isEmptyObject(redeemedDATA)) {
					...
					if(!isEmpty(ECTQty) && ECTQty > 1){
					 ...
					 if (!isEmpty(ECTRedeemedAmt) && ECTRedeemedAmt.indexOf('.') != -1) {
						ECTRedeemedAmt = ECTRedeemedAmt.split(".")[0];
					 }
					 
					// var eCertDetails = redeemedDATA.updatedPostDetails.eCertDetails;
                     ...
                     var emailBody = '';
                     var emailSubject = "A $"+ECTDisbAmt+" eCertificate for you!"
					 ...
					 if (!isEmpty(ECTNumber)) {
					 	ecrtificateDetails +="<p class='ecert-code-lbl'>eCertificate Code:</p><p class='ecert-code-num' id='ECTnum'>"+ECTNumber+"</p>"
						$("#ecertTouchInfo").html("TIP: Touch the eCertificate number to copy it."); 	
						 if (device.platform == "Android")	{	
					 ...
					 if (!isEmpty(ECTPin)) {
					 	ecrtificateDetails +=" <p class='ecert-code-lbl'>PIN:</p><p class='ecert-code-num' id='pintouch'>"+ECTPin+"</p>"
					 	$("#ecertTouchInfo").html("TIP: Touch the eCertificate or the PIN number to copy it.");
					 	if (device.platform == "Android")	{	
					 ...
					 if (!isEmpty(ECTDisbAmt) && !isEmpty(ECTRedeemedAmt)) {
					 	$("#ECTdisbAmts").html("$"+ECTDisbAmt+" eCertificate for $"+ECTRedeemedAmt);
					 	  if (device.platform == "Android")	{		
                          	emailBody += "%0AeCertificate Value: $" + ECTDisbAmt; 
                          }else{
                          	emailBody += "eCertificate Value: $" + ECTDisbAmt + "<br>"; 
                          }
					 }
					 ...
					 if (!isEmpty(ECTRedeemIns)) {
					 	ECTRedeemIns = ECTRedeemIns.replace(/<\/?[^>]+(>|$)/g, "");//removing html tags from EcertConfInstruction
					 	if (device.platform == "Android")	{
					 		emailBody += "%0ARedemption Instructions: %0A" + ECTRedeemIns; 	
					 	}else{
					 		emailBody += "Redemption Instructions: <br>" + ECTRedeemIns; 	
					 	}							 
					 }
					 
					 console.log((dfs.crd.rwd.availablePartners + "" + ECTModeCd + ".png"));
					 $("#partnerImg").attr('src',(dfs.crd.rwd.availablePartners + "" + ECTModeCd + ".png"));
					 
					 console.log("ECTDisbAmt::"+ECTDisbAmt+"::ECTRedeemedAmt::"+ECTRedeemedAmt+"::ECTOrderId::"+ ECTOrderId + "::ECTModeDesc::"+ECTModeDesc+
					 "ECTExiprationDt :: "+ ECTExiprationDt+"::ECTNumber:; "+ ECTNumber+"::ECTPin:: "+ ECTPin);
                     ...
                     $("#redeemPrtnerTNC").click(function(){	
					  ...
					  if (device.platform === "Android")	{
					        ...
					        $("#printEcert").click(function(){		
							     navigation('../rewards/redeem_ecert_printphotos');									
					        });
                      }
					  ...
					  if (device.platform == "Android")	{					  		
					    	...
					    	emailBody = emailBody.replace(/&/g,"%26");
					  		...
					  		var mailToStr = "mailto:?subject="+emailSubject+"&body="+emailBody;
							$("#emailEcert").attr("href", mailToStr);
							
					  }else{
					  	   console.log("for IOS add native plugin call");
						   $("#emailEcert").click(function(){	
						   		console.log("emailllll" + emailBody);	
	                                              console.log("email");
	                            EmailComposer.prototype.showEmailComposer(emailSubject,emailBody,"",null,null,"true");                   
	...
	$("#ecart-ios").live('pageshow',function(){
		...
		if (device.platform == "Android")	{
		    ...
		    $("p.ecert-code-num").click(function(){
		        	var pinText = $(this).html();                               
		       		 console.log("clip" + pinText);
		        	clipboardPlugin.prototype.setText(function(s){console.log('copied to clipboard');}, pinText);
		        });
		 }
    });
	...
	function redeem_ecert_printphotosLoad(){
		...
		try{
			...
			var validPriorPagesOfPrint= new Array("redeemPartnerECT3","redeemCashbackEcertConfDetails");
			if(jQuery.inArray(fromPageName, validPriorPagesOfPrint) > -1 ){	
				if (!jQuery.isEmptyObject(printData)) {
				    ...
				    var printECTPin = printData.eCertPin;
					 ...
					 if (!isEmpty(printRedeemedAmt) && printRedeemedAmt.indexOf('.') != -1) {
					 ...
					 if (!isEmpty(printDisbAmt) && !isEmpty(printRedeemedAmt)) {
					 	$("#pECTdisbAmts").html("$"+printDisbAmt+" eCertificate for $"+printRedeemedAmt);					 
					 }
					 if (!isEmpty(printModeDesc)) {
					 	$("#pECTdesc").html(printModeDesc);
					 }
					 $("#pImg").attr('src',(dfs.crd.rwd.availablePartners + "" + printModeCd + ".png"));
					 
					 var ecrtificateDetails = '';
					 if (!isEmpty(printEertNumber)) {
					 	ecrtificateDetails +="<p class='ecert-code-lbl-savetophotos'>eCertificate Code:</p><p class='ecert-code-num-savetophotos' id='ECTpin'>"+printEertNumber+"</p>"
					 }
					 if (!isEmpty(printECTPin)) {
					 	ecrtificateDetails +=" <p class='ecert-code-lbl-savetophotos'>PIN:</p><p class='ecert-code-num-savetophotos'>"+printECTPin+"</p>"
					  }
					 ...
					 $("#pectdisbAmt").html("$"+printDisbAmt);	
					  $("#cancelPrint").click(function(){		
					   ...
					   $("#printPhoto").click(function(){		
                           ...
                           // var printDivHtmlContent1 = document.getElementById("ecart-print-pin").innerHTML;
                            //alert(printDivHtmlContent1);                                                                       
                          	window.plugins.Print.print(function(s){console.log("success");}, function(e){console.log("ERROR");}, printDivHtmlContent, null, null);  
	...
	function redeem_ecart_savetophotos_pinLoad(){
		...
		try{
			...
			var validPriorPagesOfSave= new Array("redeemPartnerECT3","redeemCashbackEcertConfDetails");
			if(jQuery.inArray(fromPageName, validPriorPagesOfSave) > -1 ){	
				if (!jQuery.isEmptyObject(printData)) {
				    ...
				    var printECTPin = printData.eCertPin;
					 ...
					 if (!isEmpty(printDisbAmt) && !isEmpty(printRedeemedAmt)) {
					 	$("#sECTdisbAmts").html("$"+printDisbAmt+" eCertificate for $"+printRedeemedAmt);					 
					 }
					 if (!isEmpty(printModeDesc)) {
					 	$("#sECTdesc").html(printModeDesc);
					 }
					 $("#spImg").attr('src',(dfs.crd.rwd.availablePartners + "" + printModeCd + ".png"));
					 var ecrtificateDetails = '';
					 if (!isEmpty(printEertNumber)) {
					 	ecrtificateDetails +="<p class='ecert-code-lbl-savetophotos'>eCertificate Code:</p><p class='ecert-code-num-savetophotos' id='ECTpin'>"+printEertNumber+"</p>"
					 }
					   ...
					   $("#savePhoto").click(function(){		
							...
							if (device.platform == "Android"){
	                            ...
	                            if( !$('#removethisdiv').is(':visible') ) {
									console.log("hidden");
									window.plugins.Screenshot.takeScreenshot(function() { console.log("screenshot success"); }, function(e) {console.log("error: " + e);} );
								}
							}
							else{
	                            ...
	                            if( !$('#removethisdiv').is(':visible') ) {
									console.log("hidden");
									window.plugins.Screenshot.takeScreenshot();
								}
	...
	function redeemInstructionsLoad(){
		try{
			...
			if(!isEmpty(instructionData)){
				...
				var isMobilized = instructionData.isMobilized;
				var partnerDesc = instructionData.modeDescShort;
				
				console.log(":isMobilized:" + isMobilized); 
				console.log(":instructionData.partnerPhone:" + instructionData.partnerPhone); 
				...
				if (!isEmpty(isMobilized) && isMobilized === true){
					 console.log("mobilized partner");
					 $("#noteToCashier").html("<strong>Note to Cashier:</strong> Please process as a store gift card.");
					 $("#noprinting").html("<strong>No Printing Necessary!: </strong>"+partnerDesc+" eCertificate is able to be used in store right from your mobile device.  Please show this eCertificate to the cashier at time of purchase.");
				}
				if (!isEmpty(instructionData.partnerPhone) && instructionData.partnerPhone !== null) {
					console.log("callable partner");
					var partnerPhnHref = "tel:"+instructionData.partnerPhone;
					$("#callable").html("<a href='"+partnerPhnHref+"' data-role='button' data-inline='false' class='common-btn'>Click to Call Now</a>");
					
				}else{
					$("#callable").html("<a href='#' data-role='button' data-inline='false' data-rel='back' class='common-btn'>Done</a>");
			...
			if(jQuery.inArray(fromPageName, validPriorPagesOfGCD3) > -1 ){		
				...
				console.log("post detials present");
					 ...
					 if(!isEmpty(redeemedData.updatedPostDetails.modeDesc)){
					 	$("#gcdModeDesc").html(redeemedData.updatedPostDetails.modeDesc);
					 }
					 if(!isEmpty(redeemedData.updatedPostDetails.orderId)){
					 	$("#gcdOrderId").html("Order #:" + redeemedData.updatedPostDetails.orderId);
					 }
					 if(!isEmpty(redeemedData.updatedPostDetails.expDate)){
					 	$("#gcdExpDate").html("Expiration: " + redeemedData.updatedPostDetails.expDate)
					 }
					 console.log((dfs.crd.rwd.availablePartners + "" + gcdModeCode + ".png"));
					 $("#partnerImg").attr('src',(dfs.crd.rwd.availablePartners + "" + gcdModeCode + ".png"));
	...
	dfs.crd.rwd.postCBB = function(merchantDetailsToPOST){
		...
		try {		
			if (!jQuery.isEmptyObject(merchantDetailsToPOST)) {
				var newData = new Date();
				var redeemMerchantURL = RESTURL + "rewards/v2/redeem";	
				...
				var dataJSON={"amount":amount,"mediaCode":mediaCode,"modeCode":modeCode,"orderQty":orderQty};
	...
	function redeemBestValueLoad(){
		...
		try{
			...
			if (!jQuery.isEmptyObject(bestValuePartnersData)) {
				...
				//var unavailExtraStr = "<p class='disabledTxt ui-li-desc'>You don't have enough <i>Cashback Bonus</i> to redeem at this merchant.Keep earning!</p>";
				var pg_extra = "fromALL";		
				
				
						
				if (extra20OrMoreList !== null && extra20OrMoreList !== 'undefined' && extra20OrMoreList !== '')	{					
							
					console.log("20 list length" + extra20OrMoreList.length);		
					var extra20ULList = "<ul class='suggestions' data-role='listview' >";					
					$.each (extra20OrMoreList, function (extraTwenty) {	
							...
							var unavailExtraStr = "<p> You're only $"+extra20missingAmt+" <i>Cashback Bonus</i> away from being able to get $"+extra20minRedemptionAmount+" for $"+extra20minWhatYouGetAmount+".</p>";
							
							if(extra20OrMoreList[extraTwenty].isPartnerAvail === true){ // for available partners
								activeExtraLi += "<li>";
								activeExtraLi += "<span class='fLeft'>";
								activeExtraLi += "<a href='#' onClick=dfs.crd.rwd.renderPartnerDetails('"+ pModeCode + "','"+ pg_extra + "')>";
								activeExtraLi += "<img src='"+ dfs.crd.rwd.availablePartners + ""+ pModeCode + ".png' width='104' height='56'/>";
								activeExtraLi += "</span>";
								activeExtraLi += " <span class='giftName'>"+pModeDesc+"</span></a>";
								...
								activeExtraLi += "<img src='"+ dfs.crd.rwd.unavailablePartners + ""+ pModeCode + ".png' width='104' height='56'/>";
								activeExtraLi += "</span>";
								activeExtraLi += " <span class='giftName'>"+pModeDesc+"</span>";
								//activeExtraLi += unavailExtraStr;
								activeExtraLi += "<p class='disabledTxt'>You're only $"+extra20missingAmt+" <i>Cashback Bonus</i> away from being able to get $"+extra20minRedemptionAmount+" for $"+extra20minWhatYouGetAmount+".</p>";
				...
				$("#amnt15").click(function(){		
					console.log("extra 15 clicked");
					var extra15List = bestValuePartnersData.extra15;
					if (extra15List !== null && extra15List !== 'undefined' && extra15List !== '')	{
					
						
						console.log("15 list length :::::::" + extra15List.length);	
						var extra15ULList = "<ul class='suggestions' data-role='listview' >";				
						$.each (extra15List, function (extra15) {	
								...
								if(extra15List[extra15].isPartnerAvail === true){ // for available partners
									activeExtra15Li += "<li>";
									activeExtra15Li += "<span class='fLeft'>";
									activeExtra15Li += "<a href='#' onClick=dfs.crd.rwd.renderPartnerDetails('"+ p15ModeCode + "','"+ pg_extra + "')>";
									activeExtra15Li += "<img src='"+ dfs.crd.rwd.availablePartners + ""+ p15ModeCode + ".png' width='104' height='56'/>";
									activeExtra15Li += "</span>";
									activeExtra15Li += " <span class='giftName'>"+p15ModeDesc+"</span></a>";
									...
									activeExtra15Li += "<img src='"+ dfs.crd.rwd.unavailablePartners + ""+ p15ModeCode + ".png' width='104' height='56'/>";
									activeExtra15Li += "</span>";
									activeExtra15Li += " <span class='giftName'>"+p15ModeDesc+"</span>";
									activeExtra15Li += "<p class='disabledTxt'>You're only $"+p15missingAmt+" <i>Cashback Bonus</i> away from being able to get $"+p15minRedemptionAmount+" for $"+p15minWhatYouGetAmount+".</p>";
									///activeExtra15Li += "<p class='disabledTxt'>You don't have enough <i>Cashback Bonus</i> to redeem at this merchant.Keep earning!</p>";
						  ...
						  extra15ULList +="</ul>";
						  console.log(" 15 waali extraULList " + extra15ULList)
				...
				$("#amnt10").click(function(){	
						console.log("extra 10 clicked");
						var extra10List = bestValuePartnersData.extra10;
				
						if (extra10List !== null && extra10List !== 'undefined' && extra10List !== '')	{
						
								
							console.log("10 list length :::::::" + extra10List.length);	
							var extra10ULList = "<ul class='suggestions' data-role='listview' >";					
							$.each (extra10List, function (extra10) {	
								...
								if(extra10List[extra10].isPartnerAvail === true){ // for available partners
									activeExtra10Li += "<li>";
									activeExtra10Li += "<span class='fLeft'>";
									activeExtra10Li += "<a href='#' onClick=dfs.crd.rwd.renderPartnerDetails('"+ p10ModeCode + "','"+ pg_extra + "')>";
									activeExtra10Li += "<img src='"+ dfs.crd.rwd.availablePartners + ""+ p10ModeCode + ".png' width='104' height='56'/>";
									activeExtra10Li += "</span>";
									activeExtra10Li += " <span class='giftName'>"+p10ModeDesc+"</span></a>";
									...
									activeExtra10Li += "<img src='"+ dfs.crd.rwd.unavailablePartners + ""+ p10ModeCode + ".png' width='104' height='56'/>";
									activeExtra10Li += "</span>";
									activeExtra10Li += " <span class='giftName'>"+p10ModeDesc+"</span>";
									activeExtra10Li += "<p class='disabledTxt'>You're only $"+p10missingAmt+" <i>Cashback Bonus</i> away from being able to get $"+p10minRedemptionAmount+" for $"+p10minWhatYouGetAmount+".</p>";
									///activeExtra10Li += "<p class='disabledTxt'>You don't have enough <i>Cashback Bonus</i> to redeem at this merchant.Keep earning!</p>";
				...
				$("#amnt5").click(function(){	
						...
						if (extra5List !== null && extra5List !== 'undefined' && extra5List !== '')	{
							...
							$.each (extra5List, function (extra5) {	
								...
								if(extra5List[extra5].isPartnerAvail === true){ // for available partners
									activeExtra5Li += "<li>";
									activeExtra5Li += "<span class='fLeft'>";
									activeExtra5Li += "<a href='#' onClick=dfs.crd.rwd.renderPartnerDetails('"+ p5ModeCode + "','"+ pg_extra + "')>";
									activeExtra5Li += "<img src='"+ dfs.crd.rwd.availablePartners + ""+ p5ModeCode + ".png' width='104' height='56'/>";
									activeExtra5Li += "</span>";
									activeExtra5Li += " <span class='giftName'>"+p5ModeDesc+"</span></a>";
									...
									activeExtra5Li += "<img src='"+ dfs.crd.rwd.unavailablePartners + ""+ p5ModeCode + ".png' width='104' height='56'/>";
									activeExtra5Li += "</span>";
									activeExtra5Li += " <span class='giftName'>"+p5ModeDesc+"</span>";
									activeExtra5Li += "<p class='disabledTxt'>You're only $"+p5missingAmt+" <i>Cashback Bonus</i> away from being able to get $"+p5minRedemptionAmount+" for $"+p5minWhatYouGetAmount+".</p>";
									///activeExtra5Li += "<p class='disabledTxt'>You don't have enough <i>Cashback Bonus</i> to redeem at this merchant.Keep earning!</p>";
	...
	dfs.crd.rwd.getAllBestValuePartnerDetails = function(){
		...
		try {
			...
			var newData = new Date();
			var ALLBESTVALUEPARTNERSURL = RESTURL + "rewards/v2/bestvalue?rewardType=ALL&"
			+ newData + "";	
			...
			if (jQuery.isEmptyObject(redeemHistory)) {
				...
				$.ajax({
					...
					error : function(jqXHR, textStatus, errorThrown){
						...
						cpEvent.preventDefault();
						var code=getResponseStatusCode(jqXHR);
						console.log("error in getting redemption history" + code);
	...
	dfs.crd.rwd.populatRedeemptionHistory=function(responseData){
		...
		try{
			...
			if (!jQuery.isEmptyObject(responseData)){
				...
				if (orderHistoryList !== null && orderHistoryList !== 'undefined' && orderHistoryList !== '' && orderHistoryList.length !==0)	{	
					...
					$.each (orderHistoryList, function (order){
						...
						var isModeConfigured = orderHistory.isModeConfigured;
						 ...
						 if (!isEmpty(dsbrstAmt) && dsbrstAmt.indexOf('.') != -1) {
							dsbrstAmt = dsbrstAmt.split(".")[0];
						 }
						 if (!isEmpty(dsbrstCashAmt) && dsbrstCashAmt.indexOf('.') != -1) {
							dsbrstCashAmt = dsbrstCashAmt.split(".")[0];
						 }
						 console.log("isModeConfigured :: " + isModeConfigured);
						 if (!isEmpty(isModeConfigured) && isModeConfigured === true) {// adding for vendors like Amazon and Facebook, Itunes
							orderId = vendorOrderId;
						 }
						 					 
						 /*$("#ect").click(function(){		
							$("#ect").attr('class', 'ui-btn-active ui-state-persist border-left');
						 ...
						 switch(redeemOptionId){
							...
							case "2":// for DISCOVER GIFT CARD
								dgcItemDesc="Discover Gift Card <br /> Name on Card : " + firstLastName+"<br />Design : "+itemDesc;	
						...
						console.log("redeemOptionId :: " + redeemOptionId);
						if(!isEmpty(mediaCode) && mediaCode === "ECT" && redeemOptionId === "1"){
					
							redeemHistLi += "<li class='rd-ecert' onClick=dfs.crd.rwd.getEcetDetailsFromModeCode('"+ mediaCode + "','"+ userModeCode + "','"+ orderId + "','"+ eCertNum + "','"+ orderDate + "','"+ dsbrstAmt + "')>";
							redeemHistLi +=	"<div class='ui-grid-c'>";
							redeemHistLi +=	"<div class='ui-block-a'>";
							redeemHistLi +=	"<p><strong> "+itemDesc+" </strong></p>";
							if(accountEarnsMiles(orderhistIncentiveTypeCd)){
								redeemHistLi += "<p>$"+numberWithCommas(dsbrstCashAmtDisp)+" eCertificate for "+numberWithCommas(dsbrstAmtDisp)+" Miles</p>";
							}else{
								redeemHistLi += "<p>$"+numberWithCommas(dsbrstCashAmtDisp)+" eCertificate for $"+numberWithCommas(dsbrstAmtDisp)+"</p>";
							}
							redeemHistLi +=	"</div>";
							redeemHistLi += "<div class='ui-block-b'>";
							redeemHistLi += "<span>Order: #"+orderId+"</span><p>"+orderDate+"</p>";
							redeemHistLi += "</div>";
							redeemHistLi += "<div class='ui-block-c'><div class='ecert-icon'></div></div>";
							redeemHistLi += " <div class='ui-block-d'>";
							redeemHistLi += "<a href='#' data-transition='slide' class='rd-right-arrow' id='EecrtConfirm' onClick=dfs.crd.rwd.getEcetDetailsFromModeCode('"+ mediaCode + "','"+ userModeCode + "','"+ orderId + "','"+ eCertNum + "','"+ orderDate + "','"+ dsbrstAmt + "')></a></div>"
							//redeemHistLi += "<div class='ui-block-d rd-right-arrow' id='EecrtConfirm' onClick=dfs.crd.rwd.getEcetDetailsFromModeCode('"+ mediaCode + "','"+ userModeCode + "','"+ orderId + "','"+ eCertNum + "','"+ orderDate + "')></div>"
							redeemHistLi += "</div>";
							redeemHistLi += "</li>";
							redeemHistUL += redeemHistLi;
											
						}
						else if(!isEmpty(mediaCode) && mediaCode === "GCD" && redeemOptionId === "1"){
							
							console.log("building gift cards hist L:: " );
							redeemHistLi += "<li>";
							redeemHistLi +=	"<div class='ui-grid-c'>";
							redeemHistLi +=	"<div class='ui-block-a'>";
							redeemHistLi +=	"<p><strong> "+itemDesc+" </strong></p>";
							if(accountEarnsMiles(orderhistIncentiveTypeCd)){
								redeemHistLi += "<p>$"+numberWithCommas(dsbrstCashAmtDisp)+" Gift Card for "+numberWithCommas(dsbrstAmtDisp)+" Miles</p>";
							}else{
								redeemHistLi += "<p>$"+numberWithCommas(dsbrstCashAmtDisp)+" Gift Card for $"+numberWithCommas(dsbrstAmtDisp)+"</p>";
							}
							redeemHistLi +=	"</div>";
							redeemHistLi += "<div class='ui-block-b'>";
							redeemHistLi += "<span>Order: #"+orderId+"</span><p>"+orderDate+"</p>";
							...
							redeemHistLi +=	"<p><strong> "+itemDesc+" </strong></p>";
							if(accountEarnsMiles(orderhistIncentiveTypeCd)){
								redeemHistLi += "<p>"+numberWithCommas(dsbrstAmtDisp)+" Miles</p>";
							}else{
								redeemHistLi += "<p>$"+numberWithCommas(dsbrstAmtDisp)+"</p>";
							}
	...
	dfs.crd.rwd.getEcetDetailsFromModeCode=function(mediaCode,userModeCode,orderId,eCertNo,date,redeemAmt){	
		console.log("getEcetDetailsFromModeCode");
		var eCertData=dfs.crd.rwd.getEcertPartnerRedeemHistory(mediaCode,userModeCode,orderId,eCertNo,date,redeemAmt);
	...
	dfs.crd.rwd.getEcertPartnerRedeemHistory=function(mediaCode,userModeCode,orderId,eCertNo,date,redeemAmt){
	console.log("getEcertPartnerRedeemHistory redeem amount == " + redeemAmt);
		try{
			var ecertPartnerConfDetails;
			var newData = new Date();	
			var ECERTPARTNERSCONFURL =RESTURL+"rewards/v2/orderhistory?mediaCode="+mediaCode+"&modeCode="+userModeCode+"&eCertNumber="+eCertNo+"&orderId="+orderId+"&orderDate="+date+"&"+newData;
			...
			$.ajax({
				...
				success : function(responseData, status, jqXHR) {
					...
					if (jqXHR.status != 200 & jqXHR.status != 204) {
						...
						putDataToCache("ECERT_REDEEM_HISTORY_DETAILS", ecertPartnerConfDetails);
					}
				},
				error : function(jqXHR, textStatus, errorThrown) {
	...
	function redeemCashbackEcertConfDetailsLoad(){
		try{
			...
			if (!jQuery.isEmptyObject(redeemECTConfData)){
					...
					console.log("ECTConfDisbAmt :: "+ ECTConfDisbAmt + "::	ECTConfRedeemedAmt ::"+ ECTConfRedeemedAmt);	
					console.log("isMobilized : " + redeemECTConfData.isMobilized);
					console.log("partnerPhone : " + redeemECTConfData.partnerPhone);				
					 if (!isEmpty(ECTConfDisbAmt) && ECTConfDisbAmt.indexOf('.') != -1) {
						ECTConfDisbAmt = ECTConfDisbAmt.split(".")[0];
					 }
					if (!isEmpty(ECTConfRedeemedAmt) && ECTConfRedeemedAmt.indexOf('.') != -1) {
					 ...
					 var emailBody = '';
					 var emailSubject = "A $"+ECTConfDisbAmt+" eCertificate for you!"
					 ...
					 $("#partnerImg").attr('src',(dfs.crd.rwd.availablePartners + "" + ECTConfModeCd + ".png"));
					 
					 var ecrtificateConfDetails = '';
					 if (!isEmpty(ECTConfNumber)) {
					 	ecrtificateConfDetails +="<p class='ecert-code-lbl'>eCertificate Code:</p><p class='ecert-code-num' id='ECTpin'>"+ECTConfNumber+"</p>"
					 ...
					 if (!isEmpty(ECTConfPin)) {
					 	ecrtificateConfDetails +=" <p class='ecert-code-lbl display-inline'>PIN:</p><p class='ecert-code-num display-inline'>"+ECTConfPin+"</p>"
                        if (device.platform == "Android"){
					  ...
					  if (!isEmpty(ECTConfDisbAmt) && !isEmpty(ECTConfRedeemedAmt)) {
					 	$("#ECTConfdisbAmts").html("$"+ECTConfDisbAmt+" eCertificate for $"+ECTConfRedeemedAmt);
					 	 if (device.platform == "Android")	{
                         	emailBody += "%0AeCertificate Value: $" + ECTConfDisbAmt; 
                         }else{
                         	emailBody += "eCertificate Value: $" + ECTConfDisbAmt + "<br>"; 
                         }
					 }
					 ...
					 if(!isEmpty(EcertConfInstruction)){
					 	EcertConfInstruction = EcertConfInstruction.replace(/<\/?[^>]+(>|$)/g, "");//removing html tags from EcertConfInstruction
					 	//EcertConfInstruction = EcertConfInstruction.replace(/#/g,"");
					 	//EcertConfInstruction = EcertConfInstruction.replace(/&/g,"");
					 	//EcertConfInstruction = EcertConfInstruction.replace(/;/g,"");
					 	if (device.platform == "Android"){		
					  		emailBody += "%0ARedemption Instructions:%0A" + EcertConfInstruction; 	
					  	}else{
					  		emailBody += "Redemption Instructions: <br>" + EcertConfInstruction; 	
					  	}				  	
