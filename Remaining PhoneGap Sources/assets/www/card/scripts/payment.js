		...
		if(!isEmpty(paymentHistory)){
			...
			if (innerHisTransVal.length != 0) {
				...
				for (i = 0; i < counter; i++) {
					...
					var paymentHistoryDataActivityJSON = "<li class ='borderbottom'><div class='accountsearchresult'><div class='toprow1'>!~bank~!</div><div><div class='col1'>!~paymentDate~!</div><div class='col2'>!~paymentMethod~!</div><div class='col3 boldtext'>$!~paymentAmount~!</div></div></div></li>";
...
function paymentStep1Load()
{
	try {
		...
		if (jQuery.inArray(fromPageName, validPriorPagesOfpayStep1) > -1) {
		...
		.ajax(
				{
					...
					success : function(responseData, status, jqXHR)
					{
						...
						hideSpinner();
						/*
						 * if (jqXHR.status != 200) { var
						 * code=getResponseStatusCode(jqXHR); switch(code){
						 * case '1219': var
						 * errorMsgData=getResponsErrorData(jqXHR);
						 * $('#commonErrorInMakePaymentStepOneDiv').html(
						 * errorCodeMap['1219']); if(!isEmpty
						 * (errorMsgData[0])){ stepOne = errorMsgData[0];
						 * }else{ errorHandler(code, '', 'PaymentStep1'); } } }
						 * else { stepOne = responseData;
...
dfs.crd.pymt.paymentDateSet = function(paymentDate)
{
	try {
		...
		$("#datepicker-val").show().text(formattedDate);
		$("#datepicker-val").css("margin","4px 0px");
		$("#date-compare").val(paymentDate);
