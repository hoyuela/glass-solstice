		...
		_create: function(){
			...
			$el.find('.inc, .dec').click(function(){
				...
				if ($btn.hasClass('inc')){
					...
					var newVal = oldVal == maxQty ? maxQty : oldVal + parseInt(o.step);
					
					if(newVal >= maxQty){
						$(".ui-corner-right").addClass("disabled-btn");
						if(typeof maxQty != 'undefined' && typeof minQty != 'undefined' && maxQty != minQty){
					...
					var newVal = oldVal == minQty ? minQty : oldVal - parseInt(o.step);
					if(newVal <= minQty){
						$(".ui-corner-right").removeClass("disabled-btn");
						$(".ui-corner-left").addClass("disabled-btn");
						if(typeof maxQty != 'undefined' && maxQty != minQty){
							$(".ui-corner-right").removeClass("disabled-btn");
						}						
					}else{
						$(".ui-corner-right").removeClass("disabled-btn");
