...
dfs.crd.lilo.isDeviceRegistered = function()
{
	try{
	 ...
	 //13.1 change for C2DM to GCM Android migration
        var deviceT="";
        if (!isEmpty(deviceType)) deviceT=deviceType.toLowerCase();
        
        if (localStorage.getItem("GCMMigration") != "TRUE" && deviceT == "android")
        {
            localStorage.clear();
            localStorage.setItem("GCMMigration", "TRUE");
        }
...
dfs.crd.lilo.validateSSOData = function(cardHome) {
	return !jQuery.isEmptyObject(cardHome) && typeof(cardHome.isSSOUser) === "boolean" && cardHome.isSSOUser && typeof(cardHome.payLoadSSOText) === "string" && cardHome.payLoadSSOText.length > 0;
}

dfs.crd.lilo.goToBankSSO = function(ssoToken)
{
	try {
		$(window).bind('beforeunload', function() {
			// spinner is shown in achome.js
			hideSpinner();
		});
		
		// Emergency spinner hider timer.  Called if the transition to
		// bank hangs for some reason.
		window.setTimeout(function() {
			hideSpinner();
		}, 10000);
	
		window.location.href = BANK_SSO_URL + ssoToken;
    } catch (err) {
		hideSpinner();
        showSysException(err);
    }
...
{		
	try{
		...
		/*pushRedirection = false; // Push Requirement
		clearGlobalCache();	
...
dfs.crd.lilo.errorHandlerLogin = function(errorCode, customErrorMessage,strongAuthOrUserID,label, buttonLabel, buttonUrl, buttonExternal)
{
	try{
		var errorMsg;	
		...
		case "1633"	  :
			$("#loginErrorPage-Pg").live("pagebeforeshow",function(){
				...
				// New button code
				var button = errorCodeMap[errorCode + "_BUTTON"];
				if (!!button) {
					if (buttonExternal) {
						$("#globalErrorActionButton").html("<br/><div align='center'><a href='" + buttonUrl + "' target='_new' data-role='button' id='js-submit' data-theme='c' data-corners='true' data-shadow='true' data-iconshadow='true' data-wrapperels='span' class='ui-btn ui-shadow ui-btn-corner-all ui-btn-up-c'><span class='ui-btn-inner ui-btn-corner-all'><span class='ui-btn-text'>" + buttonLabel + "</span></span></a></div>");
					} else {
						$("#globalErrorActionButton").html("<br/><div align='center'><a href='" + buttonUrl + "' data-role='button' id='js-submit' data-theme='c' data-corners='true' data-shadow='true' data-iconshadow='true' data-wrapperels='span' class='ui-btn ui-shadow ui-btn-corner-all ui-btn-up-c'><span class='ui-btn-inner ui-btn-corner-all'><span class='ui-btn-text'>" + buttonLabel + "</span></span></a></div>");
					}
				}
		...
		if (dfs.crd.lilo.validateLoginParams(user,password,rememberID))
		{
			...
			$.ajax({
				...
				success : function(response, status, jqXHR){
					hideSpinner();
                   if (!validateResponse(response,"accountValidation"))      // Pen Test Validation
                   {
                   errorHandler("SecurityTestFail","","");
                   return;
                   }
					getSecToken();
                   //CookieManager.prototype.getCookie(function successToken (arg) {dfsKey = arg; postSiteCatGlobalData();}, null, "dfsedskey");
					dfs.crd.lilo.userLogOut = false;	
					...
					if (jQuery.isEmptyObject(response)){
						...
						dfs.crd.lilo.successfulLogin(response);
					}
				},
				...
				error : function(jqXHR, textStatus, errorThrown){
					...
					var stausCode=getResponseStatusCode(jqXHR);
						...
						if( 0 != stausCode ){
							...
							case "1102":
								var ssoUser = false;
								var delinkable = false;
								if (!isEmpty(errorMsgData) && errorMsgData.length >= 2 && errorMsgData[0].hasOwnProperty("isSSOUser") && errorMsgData[1].hasOwnProperty("isSSOUidDLinkable")) {
									ssoUser = errorMsgData[0].isSSOUser;
									delinkable = errorMsgData[1].isSSOUidDLinkable;
								}
								
								if (ssoUser && !delinkable) {
									errorCode = errorCode + "_SS0";
									buttonlabel = "Log In"
									url = BANK_LUA_URL;
									external = false;
								}
								...
								if (ssoUser && delinkable) {
									errorCode = errorCode + "_SSO_DELINK";
					...
					dfs.crd.lilo.errorHandlerLogin(errorCode, userIdDisplayText,"",label, buttonlabel, url, external);
				}
...
dfs.crd.lilo.successfulLogin = function(response) {
			...
			if(applicationMode == null){
                        ...
                        $.ajax({
                          ...
                          if (resultCode == "F"){
                               navigation('../pushNotification/pushTermsAndConditions');
								dfs.crd.sct.deviceTermsNConditions();
...
dfs.crd.lilo.gotoACHome = function(error)
{
    ...
    if (pushRedirection && (!dfs.crd.lilo.otherUser)) {
		...
		if ($.mobile.activePage !== null) {
			console.log("Active page url: " + $.mobile.activePage.attr("data-url"));
		}
		console.log("Path: " + document.location.pathname);
		if ($.mobile.activePage == null && document.location.pathname.indexOf("index.html") > 1) {
			navigation("card/html/achome/cardHome");
		} else if ($.mobile.activePage !== null && $.mobile.activePage.attr("data-url") === "login-pg") {
			console.log("going to long url, sso?");
			navigation("card/html/achome/cardHome");
		} else {
			navigation('../achome/cardHome');
		}
...
dfs.crd.lilo.preLoginCheck = function(successCallback){
    try{
		if (typeof(successCallback) !== "function") {
			successCallback = dfs.crd.lilo.goToCardLogin;
		}
...
dfs.crd.lilo.cancelVersionUpdate = function(){
	...
	dfs.crd.sct.cancelLink();//passing sitecatalyst variable for cancelling version update
}
...
dfs.crd.lilo.doSSOLogin = function() {

	// We try to get ACHome data to verify if the user has a valid session.  If they do,
	// we just navigate to ACHome.
	var cardHome = dfs.crd.achome.getACHomeData('ACHOME');
	try {
		if (!!cardHome) {
			//var check = dfs.crd.lilo.preLoginCheck();
			// TODO - we are forcing the prelogin check to true as a temporary workaround.
			var check = true;
			if (!!check) {
				$(window).one("pageshow", function() {
					dfs.crd.lilo.successfulLogin(cardHome);
				});
				
				$("#login-pg").live("pageshow", function() {
					$(this).hide();
				});
				
				$("#login-pg").live("pagehide", function() {
					$(this).show();
				});
			...
			console.log("No data, going to the normal login page");
			return false;
		}
	} catch(err) {
		console.log(err);
	...
	// Make sure something gets returned no matter what
	return false;
}
...
$("#login-pg").live("pageinit", function(event, ui) {
					...
					// Check for bare sso parameter in url
					
					
					if (window.location.search.match("[?&]sso") != null) {
						if (!dfs.crd.lilo.doSSOLogin()) {
							messageForLogout = "Sorry, we were unable to transfer you to your Card account." + 
							 "<span>Please login again.</span>";
						}
					}
...
function getParameterByName(name) {
    ...
    //return !!match && match.length > 0 && decodeURIComponent(match[1].replace(/\+/g, " "));
...
/*passing sitecatalyst variable for force upgrade and optional upgrade - start*/
function forceUpgradeLoad(){
	forceUpgradeSCVariables();
}

function optionalUpgradeLoad(){
	optionalUpgradeSCVariables();
}
/*passing sitecatalyst variable for force upgrade and optional upgrade - end*/
$(document).jqmRemoveData('edoClassStore');