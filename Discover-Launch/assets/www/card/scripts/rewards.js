		...
		if (jQuery.isEmptyObject(cashbackbonusSignupDetails)) {
			...
			$.ajax({
				...
				success : function(responseData, status, jqXHR) {
					hideSpinner();
					 if (!validateResponse(responseData,"cbbSignupValidation")) // Pen Test Validation
	                    {
	...
	try{
		...
		.ajax({
			...
			success : function(response, status, jqXHR) {
				hideSpinner();
				if (!validateResponse(response,"cbbSignupPostValidation")) // Pen Test Validation
              {
              errorHandler("SecurityTestFail","","");
              return;
              }
	...
	try{
		...
		if (jQuery.isEmptyObject(milesSignupDetails)) {
			...
			$.ajax({
				...
				success : function(responseData, status, jqXHR) {
					hideSpinner();
					if (!validateResponse(responseData,"milesSignupValidation")) // Pen Test Validation
                    {
                        errorHandler("SecurityTestFail","","");
                       return;
                    }				
...
dfs.crd.rwds.postDataenrollForSelectedMI2Promotion = function(promoCode) {
	try{
		...
		.ajax({
			...
			success : function(response, status, jqXHR) {
				hideSpinner();
				if (!validateResponse(response,"milesSignupPostValidation")) // Pen Test Validation
              {
              errorHandler("SecurityTestFail","","");
              return;
              }
