...
$("#raf-status-miles,#raf-status,#raf-status-dollar").live("pageshow", function (event,ui) {	
  ...
  /*site catalyst implementation for RAF share now and share again links - Start*/
  $('#fbBtn').click(function(event){
  	var a = event.currentTarget.id;
  	 if($("#"+a+" .bluelink:eq(1)").text() == "Share Again"){
  		 dfs.crd.sct.rafShareAgain("Facebook");
  	 }else{
  		dfs.crd.sct.rafShareNow("Facebook");
  	 }
  });
  $('#twBtn').click(function(event){
  	var a = event.currentTarget.id;
  	 if($("#"+a+" .bluelink:eq(1)").text() == "Share Again"){
  		 dfs.crd.sct.rafShareAgain("Twitter");
  	 }else{
  		dfs.crd.sct.rafShareNow("Twitter");
  	 }
  });
  $('#lnBtn').click(function(event){
  	var a = event.currentTarget.id;
  	 if($("#"+a+" .bluelink:eq(1)").text() == "Share Again"){
  		 dfs.crd.sct.rafShareAgain("LinkedIn");
  	 }else{
  		dfs.crd.sct.rafShareNow("LinkedIn");
  	 }
  });
  $('#navToRafShareWithEmail').click(function(event){
	  	var a = event.currentTarget.id;
	  	 if($("#"+a+" .bluelink:eq(1)").text() == "Share Again"){
	  		 dfs.crd.sct.rafShareAgain("Email");
	  	 }else{
	  		dfs.crd.sct.rafShareNow("Email");
	  	 }
	  });
  /*site catalyst implementation for RAF share now and share again links - End*/
	...
	try {
		...
		//populate number of shares and successes from status data
			if (("YES" == (statusData.statuspg_api_rsp.sharing.email_share_details.activity).toUpperCase()) && 
				...
				//dfs.crd.sct.rafShareAgain('email');//passing site catalyst varibles for Refer a Friend - Share Again Clicks
			} else {
				channelArray.push("EMAIL_NEITHER");
				//dfs.crd.sct.rafShareNow('email');//passing site catalyst varibles for Refer a Friend - Share Now Clicks
			}
			if ("Y" == (statusData.statuspg_api_rsp.sharing.facebook.activity).toUpperCase()){
				...
				//dfs.crd.sct.rafShareAgain('Facebook');//passing site catalyst varibles for Refer a Friend - Share Again Clicks
			} else {
				channelArray.push("FACEBOOK_NEITHER");
				//dfs.crd.sct.rafShareNow('Facebook');//passing site catalyst varibles for Refer a Friend - Share Now Clicks
			}
			if ("Y" == (statusData.statuspg_api_rsp.sharing.twitter.activity).toUpperCase()){
				...
				//dfs.crd.sct.rafShareAgain('Twitter');//passing site catalyst varibles for Refer a Friend - Share Again Clicks
			} else {
				channelArray.push("TWITTER_NEITHER");
				//dfs.crd.sct.rafShareNow('Twitter');//passing site catalyst varibles for Refer a Friend - Share Now Clicks
			}
			if ("Y" == (statusData.statuspg_api_rsp.sharing.linked_in.activity).toUpperCase()){
				...
				//dfs.crd.sct.rafShareAgain('LinkedIn');//passing site catalyst varibles for Refer a Friend - Share Again Clicks
			} else {
				channelArray.push("LINKEDIN_NEITHER");
				//dfs.crd.sct.rafShareNow('LinkedIn');//passing site catalyst varibles for Refer a Friend - Share Now Clicks
			}
		} else {
			channelArray.push("EMAIL_NEITHER");
			channelArray.push("FACEBOOK_NEITHER");
			channelArray.push("TWITTER_NEITHER");
			channelArray.push("LINKEDIN_NEITHER");
			//dfs.crd.sct.rafShareNow();//passing site catalyst varibles for Refer a Friend - Share Now Clicks	
		}
	...
	try {
		...
		if (jQuery.isEmptyObject(strongMailShareContent)) {
			...
			$.ajax({
                ...
                //getKeyValueFromJson(responseData);
					if (!validateResponse(responseData,"referAFriendShareValidation")){ // Pen Test Validation
	...
	try {
		...
		if (jQuery.isEmptyObject(strongMailStatusParms)) {
			...
			$.ajax({
				...
				success : function(responseData, status, jqXHR) {
//getKeyValueFromJson(responseData);
					if (!validateResponse(responseData,"referAFriendStatusValidation")){ // Pen Test Validation
	                	   errorHandler("SecurityTestFail","","");
							return;
						}
					if (jqXHR.status != 200) {
				...
				error : function(jqXHR, textStatus, errorThrown) {
                   hideSpinner();
					...
					switch (code) {
						case "1629":
							referFriendDelinquentSCVariables();//passing site catalyst variable for Error Tracking - Refer a Friend - Delinquent Account
...
//		STRONGMAIL_STATUS_JSONURL = "http://www.popularmedia.net/status_page.js?api_sig=fb6a0bd8465fb4fd5c7833e3e0f3f5ff&campaign_uuid=39ae999fb2dbc02459a7ede2512af659&client_uuid=5060ac00-07da-012e-e060-f4e774cfc225&user_id=danqingqueen@gmail.com";
		...
		if (jQuery.isEmptyObject(strongMailStatusResponse)) {
			$.ajax({
				...
				success : function(responseData, status, jqXHR) {
                //getKeyValueFromJson(responseData);
					if (!validateResponse(responseData,"strongMailStatusValidation")){ // Pen Test Validation
	                	   errorHandler("SecurityTestFail","","");
							return;
						}
					if (jqXHR.status != 200) {
