...
function dd(element, settings) {
	...
	var _checkDataSetting = function() {
		...
		//alert("_checkDataSetting " + _settings.visibleRows);	
	};	
	...
	var _parseOption = function(opt) {
		...
		if (opt !== undefined) {
			...
			if (attrTitle!="") {
				...
				/*13.3 global change starts: Image icons are removed from Drop Down as per the previous build*/
	...
	var _childHeight = function (val) {
		...
		if (getElement(_element).options.length > _settings.visibleRows) {
			...
			//console.log("Calculate iHeight : " + _settings.rowHeight  + " + " + margin + " * " + _settings.visibleRows);
			iHeight = ((_settings.rowHeight + margin) * _settings.visibleRows);
			//console.log("IF iHeight : " + iHeight);
		} else if (_isList) {
			iHeight = $("#" + _element).height(); //get height from original element
			//console.log("else iHeight : " + iHeight);
		};
	...
	var _adjustOpen = function () {
		var id = _getPostID("postID");//console.log(id)
		var childid = _getPostID("postChildID");
		var pos = $("#" + id).offset();
		var mH = $("#" + id).height();
		...
		if (/*(wH + st) < Math.floor(cH + mH + pos.top) || */_settings.openDirection.toLowerCase() == 'alwaysup') {
			...
			if (top < -10) {
				$("#" + childid).css({top: (parseInt($("#" + childid).css("top")) - top + 3 + st) + "px", zIndex: _settings.zIndex});
				$("#" + id).removeClass("borderRadiusBtm borderRadiusTp").addClass("borderRadius");
			};
		} else {
			$("#" + childid).css({top: top + 3 + "px", zIndex: _settings.zIndex});
			$("#" + id).removeClass("borderRadius borderRadiusBtm").addClass("borderRadiusTp");
		};
		//console.log(pos.top+'::'+$("#" + id).height())
		//hack for ie zindex
		//i hate ie :D
		if(_isIE) {
			if(msieversion()<=7) {
	...
	var _updateTitleUI = function (index, byvalue) {
		...
		if(_settings['swipTitle'] == 'true'){
			jQuery($("#" + titleid).find("." + _styles.description)).after($("#" + titleid).find("." + _styles.label));
		}
		//update icon
...
$.fn.extend({
			msDropDown: function(settings)
			{
				//
				var setting;
				if($(this).attr('swipTitle') == 'true'){
					setting = $.extend(true, {swipTitle: 'true'}, settings)
				}else{
					setting = settings
				}
				return this.each(function()
				{
					if (!$(this).data('dd')){
						//alert("msDropDown");
						var mydropdown = new dd(this, setting);
