...
dfs.crd.reg.shared.constant.error = {
	DEFAULT : {
		...
		message : "There was a problem authenticating your information."
	},
    ...
    '5xx' : {
        ...
        message : "We're sorry. Our services are unavailable at this time. We apologize for any inconvenience."
    },
    '1006' : {
    	newPage : true,
        title : "Secure Credit Card Log In",
    ...
    "1928_handset": {
        ...
        message: "<p>Due to special conditions related to your account, we can't retrieve your User ID at this time.</p><br/><p>To recover your User ID, you will need to re-register for online account access.</p><br/>",
		button: "<div align='center'><a href='#' onclick='dfs.crd.reg.nav.goToRegistration(\"cardLogin\");' data-role='button' id='js-submit' data-theme='c' data-corners='true' data-shadow='true' data-iconshadow='true' data-wrapperels='span' class='ui-btn ui-shadow ui-btn-corner-all ui-btn-up-c'><span class='ui-btn-inner ui-btn-corner-all'><span class='ui-btn-text'>Register Now</span></span></a></div>"
    },
    NOTENROLLED : {
        newPage : true,
        title : "Enhanced Account Security",
...
dfs.crd.reg.shared.util.checkHeaderForStrongAuth = function(jqXHR, callbackPage) {
    ...
    if (jqXHR.status === 401 && saAuth !== null && saAuth.indexOf("challenge") >= 0) {
        dfs.crd.sa.checkForStrongAuth(callbackPage, false)
...
**/
dfs.crd.reg.shared.util.showErrorPage = function(title, message, button) {
    $('body').delegate("#publicError-pg", 'pagebeforeshow', function() {
        $('#errorLabel').html(title);
        $('#errorDiv').html(message);
		
		if (!!button) {
			$('#errorActionButton').html(button);
		}
...
dfs.crd.reg.shared.util.showSpecificError = function(errorCode) {
	...
	if (errorCode == "1928") {
		// use a callback to get the error device-side
		error = dfs.crd.reg.getSSOForgotUserIdErrorText(errorCode);
	} else if (!isEmpty(serverErrorText) && !isEmpty(serverErrorText[errorCode])) {
		//use server
	...
	if (!isEmpty(error.newPage) && error.newPage === true) {
		//show new page
		dfs.crd.reg.shared.util.showErrorPage(error.title, error.message, error.button);
	} else {
		//show inline errors
...
dfs.crd.reg.shared.util.populateConfirmation = function(idEmail, idLast4, idUID, cacheLocation) {
	...
	if (isEmpty(responseData)) {
		...
		////console.log("idEmail: " + idEmail + ", idLast4: " + idLast4 + ", idUID: " + idUID + ", cacheLoc: " + cacheLocation);
	}
...
dfs.crd.reg.shared.util.formValidation = (function() {
    ...
    /*var validateUserOrPasswordEquality = function(isValid, $field1, $field2, isUserIdCompare) {
    	console.log("Validate User or Password");
    ...
    var validatePassword = function($passwordField, $confirmPasswordField) {
        ...
        validateField(isValid, $passwordField);
    ...
    var validateUserId = function($userIdField, $confirmUserIdField) {
        ...
        validateField(isValid, $userIdField);          
