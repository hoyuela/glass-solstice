...
/************************ ADDITIONAL FEATURES ************************
     Plugins
*/
var s_codeVersion="DF v1.2|20111116|H23.8"
...
s.linkInternalFilters="javascript:,discover.com,discovercard.com,discoverbank.com,discoverstudentloans.com,localhost"
s.linkLeaveQueryString=false
s.linkTrackVars="prop12,eVar12,prop15,eVar15"
...
function s_doPlugins(s) {
	...
	if(!s.campaign){s.campaign=s.getQueryParam('iq_id,cmpid,utm_campaign,acmpgn,asmpgn');}
	s.campaign=s.getValOnce(s.campaign,'s_campaign',0);
	
	/* featured content/internal campaign tracking */
	if(!s.eVar1){s.eVar1 = s.getQueryParam('icmpgn')};
	s.eVar1=s.getValOnce(s.eVar1,'s_v1',0);
	
	/* non-promotional link tracking */
	if(!s.prop1){s.prop1 = s.getQueryParam('intlink')};
	...
	/* copy site siection props to eVars */
	...
	/* Discover Cards cardmember visitor ID cookie read */
	s.prop15=s.c_r('dfsedskey');
	s.eVar15="D=c15";
	/* if dfsedskey cookie is present, flag user as Customer */
	if(s.c_r('dfsedskey')){s.prop6="Customer"} else {s.prop6="Prospect";}
	s.eVar6="D=c6";
	
	/* Tool usage */
	if(s.eVar4){s.prop4="D=v4";s.events=s.apl(s.events,"event9",",",2);}
	
	/* Self service transactions */
	if(s.eVar5){s.prop5="D=v5";s.events=s.apl(s.events,"event10",",",2);}
	...
	/* Conversion variable bounce rate */
	s.visitstart=s.getVisitStart('s_vs');
	if(s.visitstart&&s.visitstart==1){s.firstPage='firstpage'; }
	s.clickPast(s.firstPage,'event10','event11');
