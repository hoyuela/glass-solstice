...
/************************ ADDITIONAL FEATURES ************************
     Plugins
*/
var s_codeVersion="DF v3.0|20130719|H26.1"
if (typeof s_account === 'undefined'){ s_account="discoverglobaldev"; } /* if report suite isn't defined at the page level, send data to global dev */
...
s.linkInternalFilters="javascript:,discover.com,discovercard.com,discoverbank.com,discoverstudentloans.com,discoverpersonalloans.com,dinersclubinfonet.com,discoverfinancial.com,discovernetwork.com,localhost"
s.linkLeaveQueryString=false
s.linkTrackVars="prop6,eVar6,prop10,prop12,eVar12,prop13,prop15,eVar15,prop16,prop17,eVar29,eVar43,eVar60,eVar59"
...
s.formList="search,loginForm,searchForm,cbbform2,getMore,redirect-form,interceptStoreForm" /* list for forms to exclude from tracking */
...
s.eventList=""
s.usePlugins=true
s.cardFilter = new RegExp(/6(?:011|5[0-9]{2})[ -+]*[0-9]{12,18}/g);
function s_doPlugins(s) {
	/* detect and overwrite credit card numbers in URL */
	s.pageURL=(""+s.wd.location).toLowerCase();
	if(s.getQueryParam('q,srchQ').search(s.cardFilter)>=0){
		s.pageURL=s.pageURL.replace(s.cardFilter,"{credit_card}");
	}
	if(s.getQueryParam('q,srchQ','',document.referrer).search(s.cardFilter)>=0){
		s.referrer=document.referrer;
		s.referrer=s.referrer.replace(s.cardFilter,"{credit_card}");
	}
	
	/*External Campaign tracking*/
	if(!s.campaign){s.campaign=s.getQueryParam('cmpgnid');}
	if(s_account.indexOf('discoverbank')!=-1){
		if(!s.campaign){s.campaign=s.getQueryParam('campaingID,priceCde,sourceCde');}
	}
	s.campaign=s.getValOnce(s.campaign,'s_campaign',0);
	/* external campaign stacking */
	s.eVar30=s.crossVisitParticipation(s.campaign,'s_ev30','30','5','>','',0);
	
	/* featured content/internal campaign tracking */
	if(!s.eVar1){
		s.eVar1 = s.getQueryParam('scmpgn,tmclk');
		if (s.getQueryParam('tmclk')){
			s.eVar1=s.eVar1.substring(0, 35);
		};
		if(!s.eVar1 && s_account.indexOf('discoverstudentloans')!=-1){
			/* if internal campaign tracking code isn't already set and we're in student loans RS, then grab value of acmpgn */
			s.eVar1 = s.getQueryParam('acmpgn');
		}
	};		
	...
	if(!s.list2) {
		...
		if(s.list2) { s.events=s.apl(s.events,"event59",",",2);	}
	}
	
	/* balance transfers click-throughs */
	if(!s.list3) {
		s.list3=s.getQueryParam('btclk'); 
		if(s.list3) { s.events=s.apl(s.events,"event67",",",2);	}
	}
	
	/* non-promotional link tracking */
	if(!s.prop1){s.prop1 = s.getQueryParam('lnk,icmpgn')};
	/* link tracking stacking */
	if(!s.eVar31){s.eVar31=s.crossVisitParticipation(s.getQueryParam('icmpgn,scmpgn'),'s_ev31','1','5','>','',0);}
	
	/* cross site tracking */
	if(!s.eVar25) {s.eVar25=s.getQueryParam('intlink'); }
	
	/* Get number of visits in last month */
	s.prop7=s.eVar7=s.getVisitNum('m');
	
	/* Call Page Name Plugin to auto-generate page names based on URL */
	if(!s.pageName){
		s.pageName=s.getPageName();
		if(s.pageName=="" || s.pageName=="index.html"){s.pageName="home";} /* if on the homepage, set the page name to home regardless of if the path is / or /index.html */
	}
	...
	if(s.pageType=="errorPage"){s.pageName="Error:"+s.wd.location;}
	...
	if(!s.eVar15){
		if(s_account.indexOf('discoverbank')!=-1 && s.c_r('cif')){
			s.eVar15="CIF:" + s.c_r('cif');
		} else {
			s.eVar15=s.c_r('dfsedskey');
		}
	} 
	...
	/* Tool usage */
	if(s.eVar4){s.prop4="D=v4";s.events=s.apl(s.events,"event8",",",2);}
	...
	if(s.eVar3) {
		...
		s.prop3=s.getAndPersistValue(s.prop3,'c3',0); /* persist prop3 to do breakdown on results clicked */
		s.eVar27=omn_getSearchType('proxystylesheet');	/* public vs. private search type */
		s.prop27="D=v27";
		if(s.eVar28){s.prop28="D=v28";}
	}
	s.eVar3=s.getValOnce(s.eVar3,'s_v3',0);
	if(s.eVar3) { s.events=s.apl(s.events,"event21",",",2); }
	if(!s.eVar23) {
		s.eVar23=s.getQueryParam('gcmpgn'); /* search result clicked */
	}		
	if(s.eVar23){ /* search result clicked logic */
		...
		s.prop23="D=v23";
		s.prop24="D=v24";
		s.prop3=s.getAndPersistValue(s.prop3,'c3',0); /* persist search term for breakdown */
		s.eVar27=omn_getSearchType('srchC'); /* public vs. private search type */
		s.prop27="D=v27";
	}

	/* add prodView event with event3 is set */
	if((s.events+',').indexOf('event3,')>-1){s.events=s.apl(s.events,"prodView",",",2);}
	
	/* Form Analysis */
	s.setupFormAnalysis();
	
	/* Shop Discover Search Term */
	if(!s.eVar52 && s.pageName.indexOf('shopdiscover')> -1){ s.eVar52=s.getQueryParam('s'); }

	/* read sitecatalyst visitor ID cookie */
	if(!s.prop25){s.prop25=s.c_r('s_vi');}
	...
	s.eVar29=s.getTimeParting('j','-6') + ' ' + s.getTimeParting('h','-6'); /* date and time */
	...
	if(!s.eVar57){s.eVar57 = s.getQueryParam('dmscmpgn,cmpgn,cmpgnid,vcmpgn,smpgn,asmpgn')};
}
