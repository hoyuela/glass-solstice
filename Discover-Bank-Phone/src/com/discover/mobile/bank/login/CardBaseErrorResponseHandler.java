...
public class CardBaseErrorResponseHandler implements ErrorResponseHandler {
	...
	protected void sendToErrorPage(final int errorCode, final int titleText,final int errorText) {
		final ErrorHandler handler = getErrorFieldUi().getErrorHandler();
		final AlertDialog dialog = handler.createErrorModal(errorCode, titleText, errorText);
		handler.showCustomAlert(dialog);
	}
	...
	protected void sendToErrorPage(final int errorText) {
		...
		final AlertDialog dialog = 
				handler.createErrorModal(context.getString(R.string.default_outage_header), 
