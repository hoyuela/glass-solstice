...
public class ErrorHandlerFactory {
	...
	/**
	 * Updates an ErrorHandlerUi to notify the user that an error has it occurred. It
	 * displays an error message on the error field, make the error label visible, Sets 
	 * the input fields to be highlighted in red, and clears the activity edit texts in 
	 * the getFieldsToClearAfterError() interface method.
	 * 
	 * The activity needs to implement ErrorHandlerUi for this functionality to work.
	 * 
	 * @param errorHandlerUi - Reference to an instance ErrorHandlerUi to update with errorText.
	 * @param errorText - Contains the error string to be displayed on ErrorHandlerUi.
	 */
	 ...
	 * 
	 * @param errorHandlerUi - Reference to ErrorHandlerUi that needs to be updated.
	 */
	...
	/**
	 * Handler for an HTTP 500 Internal Server error response. Creates a one button modal with the
	 * default error title, error message, and help number. The help number is determined
	 * based on whether the user is logged in there bank or card account.
	 * 
	 * @return Returns a one button modal which can be displayed by the calling activity
	 */
	...
	/**
	 * Handler for an HTTP 503 Service Unavailable error response. Creates a one button modal with the
	 * default error title, and help number. The help number is determined based on whether the user is 
	 * logged in there bank or card account.
	 * 
	 * Note: For Bank a 503 is meant for Planned and Unplanned System Maintenance.
	 * 
	 * @param errorText - Error message to display in the modal created
	 *  
	 * @return Returns a one button modal which can be displayed by the calling activity
	 */
	public ModalAlertWithOneButton handleHttpServiceUnavailableModal(final String errorText) {
		//Fetch modal title from resources
		String title = mActivity.getResources().getString(R.string.error_503_title);
	    
		ModalAlertWithOneButton modal = null;
		
		//If errorText is null then use the default error message when a 503 is received
		if( null == errorText ) {
			modal = createErrorModal(HttpURLConnection.HTTP_UNAVAILABLE, R.string.error_503_title, R.string.bank_error_503_message);
	...
	/**
	 * This function serves as a catch all error handler to NetworkServiceCall<> error responses.
	 * 
	 * @param httpErrorCode Specifies the HTTP status code received in the error response.
	 */
	 ...
	 * The help number is determined based on whether the user is logged in there bank or card account.
	 * The error message is expected to be provided by the error response.
	 * 
	 * @param errorText - Error message to display in the modal created
	 * 
	 * @return Returns a one button modal which can be displayed by the calling activity
	...
	public ModalAlertWithOneButton handleLockedOut(final ErrorHandlerUi errorHandlerUi,final String errorText) {
		//Fetch modal title from resources
		String title = mActivity.getResources().getString(R.string.error_403_title);
	    
		ModalAlertWithOneButton modal = null;
		
		//If errorText is null then use the default error message when a 403 is called during login attempt
		if( null == errorText ) {
			modal = createErrorModal(HttpURLConnection.HTTP_UNAVAILABLE, R.string.error_403_title, R.string.bank_error_403_message);
		} else {
			modal = createErrorModal(title, errorText);
		}
